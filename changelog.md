...
#### 4.101.0 - unreleased
* **FEATURE**: Added 1-argument convenience overloads: `JsonIo.toJson(srcObject)`, `JsonIo.toToon(srcObject)`, `JsonIo.toJava(json)`, and `JsonIo.fromToon(toon)`. Each delegates to the 2-argument version with `null` options (default behavior). Enables the clean Quick Start examples shown in the README.
