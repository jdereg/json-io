...
#### 4.103.0 - (Unreleased)
* **CLEANUP**: **Removed dead `restoreDepthAfterExternalValue` + unused `bodyDepth` locals from `JsonWriter` element loops (-51 LOC).** `CharStreamGenerator.restoreDepthAfterExternalValue(int)` was a narrower depth-only restore helper from an earlier stage of the JsonWriter dog-food migration — back when some element-write paths did a bare `resetForBridgeAtValueSlot()` with no surrounding snap+restore, and the caller needed to recover `gen.depth` after the inner emit. The migration has since wrapped all three remaining `resetForBridgeAtValueSlot` / `resetCurrentObjectFrame` paths (writeCustom primitive-form, writeCustom legacy-writer, writePrimitive Long-wrap bare-value) in their own narrow `snapshotForExternalValue` / `restoreAfterExternalValue` pairs; `restoreAfterExternalValue` already restores both `gen.depth` AND `contextStack[0..outerDepth]` AND calls `markValue()`, subsuming everything the narrow restore did. Every other element-write path now preserves `gen.depth` across the call via matched `writeStartXxxRaw` / `writeEndXxxRaw` push/pop pairs. Net effect: after `writeCollectionElement(element)` (and its analogues in `writeObjectArray` / `writeMap*` / `writeJsonObject*`), `gen.depth` is already at the pre-call value — the 15 `gen.restoreDepthAfterExternalValue(bodyDepth)` calls scattered across JsonWriter's element loops were no-op assignments. **Removed**: 15 `restoreDepth` call sites across `writePrimitiveArray`, `writeObjectArray`, `writeCollection`, `writeJsonObjectArray`, `writeJsonObjectCollection`, `writeMapToEnd`, both `writeMapBody` overloads, and `writeStringifiedMapBody`; 8 now-unused `final int bodyDepth = gen.currentDepth()` locals (the variable's only consumer was the restoreDepth call); the `restoreDepthAfterExternalValue(int)` method definition in `CharStreamGenerator` (no remaining callers); one empty-body `if` restructured to `if (!X) { ... }` for clarity; stale rationale comments referencing the restoreDepth pattern in `writePrimitiveArray`, `writeCollection`, `writeMap`, and `writeMapToEnd`. Net diff: -51 LOC (113 lines deleted, 62 added across the two files). All 4095 tests pass — behavior is byte-identical (the removed assignments wrote the value `gen.depth` already had).
* **PERFORMANCE**: **Factory-LMF VarHandle binding repairs `Accessor`'s primitive-field fast path — eliminates per-call wrapper allocation on POJO primitive field reads.** Fixes a latent bug: `LambdaMetafactory.metafactory` rejects `getField` MethodHandle kinds, so the primitive-lambda path added to `Accessor` has been silently dead since introduction. Every primitive-typed field accessor (`int id`, `long timestamp`, `double price`, `boolean enabled`, …) caught `LambdaConversionException`, fell back to `MethodHandle.invoke()` (returns `Object`), and **autoboxed every primitive on every read**. Static analysis missed it because the exception was caught and a non-primitive accessor was returned in its place. JFR-measured cost on `JsonPerformanceTest`: ~155 wrapper allocations per parse (63 int + 34 boolean + 34 double + 23 long + 1 float). **Fix**: build 8 factory `MethodHandle`s in static init via `LambdaMetafactory.metafactory`, one per primitive type — each with signature `(VarHandle) → XGetter`. LMF accepts these because the impl method is `findVirtual(VarHandle.class, "get", (Object) → primitive)`, an `invokeVirtual` MH. At Accessor construction time, when the existing primitive-lambda path fails (which it always does for direct field accessors), create a `VarHandle` for the field and bind it through the matching factory to produce an `XGetter` instance. The generated lambda class captures the `VarHandle` as a final field and dispatches `get(target)` via a direct `invokevirtual VarHandle.get(Object)X` — JIT intrinsifies the signature-polymorphic call and inlines the lambda dispatch. Microbenchmarked per-call cost: **~3.36 ns/getInt** (100M iterations in 336 ms), full inlining confirmed. The `asType` wrapper lives on the FACTORY (called once per field at Accessor construction), not the per-call path; the prior `asType`-on-call attempt showed `MethodHandle.invokeBasic` at 1.11% leaf in JFR, that's now gone. Existing `getInt`/`getLong`/`getBoolean`/... fast paths require no changes — they already check `primitiveFunction`; this fix just makes `primitiveFunction` non-null for primitive field accessors where it had been silently null. Method-based accessors (via `@IoGetter`) were already populating `primitiveFunction` correctly via `findVirtual` MHs and are unchanged. **JDK 8 source-compatibility preserved**: no `VarHandle` import; all VarHandle access through `Class<?>` loaded via `Class.forName` at runtime, gated by `SystemUtilities.currentJdkMajorVersion() >= 9`. JFR allocation impact on `JsonPerformanceTest` toon-write/jsonio-write: `Integer` wrappers from `Accessor.retrieve` eliminated (was 575 MB sampled-weight), `Double` wrappers eliminated (was 786 MB), `Long` wrappers reduced ~80% (residue is in `ToonWriter.formatDecimalNumber`, not in Accessor). Perf-ratio impact (median-of-3 vs `README.md`'s published 4.103.0 baseline): every one of the 12 published Jackson-ratio metrics matches or beats the prior baseline, with the biggest gains on TOON write paths (toMaps `cycleSupport=true` 1.80x → 1.74x; toJava `cycleSupport=false` 1.59x → 1.55x). All 4095 tests pass.
* **PERFORMANCE**: **Typed primitive-array inline-write loops in `ToonWriter` — eliminates per-element wrapper allocation on the all-primitives inline path.** `writeArrayElements` now dispatches to dedicated typed loops when the array's component type is primitive (`int`, `long`, `double`, `float`, `short`, `byte`, `boolean`), reading each element directly from the typed array via a checked cast rather than routing through `ArrayUtilities.getPrimitiveElement(Object, int)` + `writeInlineValue(Object)` — the prior path allocated an `Integer`/`Long`/`Double`/... wrapper per element. `char[]` is unchanged: it is intercepted upstream in `writeArray(Object)` and emitted as a plain `String`, so it never reaches the inline-array path. Output is byte-for-byte identical to the boxed path — same `toCachedLongString(long)` / `formatDecimalNumber(double|float)` / boolean ternary dispatch, just with no wrapper on the way in. JFR allocation that this change targets on the perf-test toon-write workload: ~1.8 GB `Integer` + the portion of ~2.4 GB `Long` that flowed through `getPrimitiveElement`. Same-session 2-run JsonPerformanceTest with JsonIo Write as an internal control (unchanged path → its delta = pure machine-state drift): JsonIo Write got ~2.5% slower from drift; Toon Write got 1–5% faster despite the same drift — net ~3–7% real improvement across the four Toon Write configurations, with `toMaps cs=true` (the most allocation-heavy path) showing the largest gain. The other 4.7 GB of toon-write boxing (Double/Integer from `Accessor.retrieve` reflection on POJO primitive fields) is out of scope here — it needs a typed-accessor variant in the reflection layer. All 4095 tests pass; 0 failures.
* **REFACTOR**: **Better-mix `cacheHash` for tokenizer string caches — propagates `len` through one extra multiplication.** Both `CharStreamTokenizer.cacheHash` (JSON reader) and `ToonReader.cacheHash` (TOON reader + shared with the persistent thread-local number cache) updated from `(first * 31 + mid) * 31 + last + len` to a uniform polynomial mix: `int h = first; h = h * 31 + mid; h = h * 31 + last; h = h * 31 + len;`. The added op is one IMUL + IADD per cache lookup. Measured slot-collision rate on the JsonPerformanceTest workload (477 distinct strings × ~2 lookups/parse in a 2048-slot direct-mapped cache) drops from 22.95% to 18.21% — about 2× closer to the Birthday-paradox floor for random hashing at that load factor. Hit rate moves modestly (48.4% → 49.4%); end-to-end time is within noise at this measurement precision. Empirical cache-size study run alongside: bumping to 4096 or 8192 slots did NOT meaningfully lower collisions (22.95% → 22.13% → 22.00%), confirming the hash function — not capacity — is the limiting factor. Persistent thread-local cache (warm across parses) was also measured: lifts hit rate to 68.5% but with security/memory ownership implications that are inappropriate for a general-purpose library; not shipped. All 4095 tests pass; 0 failures.
* **FEATURE**: **Identity-map sharing across `JsonGenerator.writeObject` calls.** When the same Java instance is passed to `writeObject` more than once on the same generator, the first call now emits a full serialization with a top-level `@id`, and each subsequent call emits `{"@ref":N}` pointing back at it — preserving graph identity across multiple top-level value emits. Implementation: `CharStreamGenerator` overrides `writeObject(Object)` with a lazy `IdentityHashMap<Object, Integer>` tracking emitted instances + their assigned top-level `@id`s. A per-instance counter coordinates id allocation across calls so internal sub-object ids assigned by JsonWriter inside one `writeObject` body can't collide with the gen-allocated top-level ids of other calls. Identity sharing is active when `cycleSupport(true)` (the default); under `cycleSupport(false)`, JsonWriter cannot emit `@id` at all, so this override falls back to the superclass behavior (full re-serialization on each call). Within a single call's serialization, normal intra-document cycle handling is preserved in either mode.
  * **JsonWriter changes**: `traceReferences` now preserves pre-existing entries in `objsReferenced` (was unconditionally overwriting with `identity++`). Two new package-private accessors — `currentIdentity()` / `setIdentity(int)` — let CharStreamGenerator coordinate the id namespace across calls.
  * **3 new JsonGeneratorTest cases**: `writeObject_sameInstanceTwice_secondEmitsAsRef` verifies the basic A/A → @id + @ref pattern with round-trip; `writeObject_threeCallsTwoDistinctInstances_correctIdAndRefAllocation` covers A/B/A → @id=1, @id=2, @ref=1; `writeObject_underCycleSupportFalse_doesNotEmitRefs` verifies the fallback behavior. All 4095 tests pass; 0 failures.
* **CLEANUP**: **Category-A lint sweep on JsonWriter.java.** 8 trivial cleanups: 3 unused imports (`java.lang.reflect.Type`, `CharBufScratch`, `TypeUtilities`); 2 redundant `final Writer output = this.out;` locals in `writeIntArray`/`writeShortArray` (used only once in their loops — inlined to `out.write`); 2 dangling javadoc comments deleted (orphaned blocks left from earlier refactors — one above `writeStringifiedMapBody`, one above `writeObject`); 1 unicode escape `—` replaced with the literal `—` character. Plus 2 paragraph-break javadoc fixes (`<p>` added between description paragraphs in `getTypeNameForOutput` and `shouldShowTypeForElement` to silence "blank line will be ignored"). The remaining lint warnings (raw `JsonClassWriter`/`Map`, deprecated legacy `write(Writer)` overload calls, defensive null checks, micro-perf indexed loops, public methods kept for backward compat like `newLine()`) are intentional and untouched. All 4092 tests pass; 0 failures.
* **TESTING**: **Backward-compat audit — external API surface verified against 4.102.0.** Standalone Maven test project (8 tests) exercising the public API external custom-writer authors typically use: legacy `JsonClassWriter` writer (`@Deprecated` `write(Object, boolean, Writer, WriterContext)` overload), `WriterContext` methods (writeFieldName, writeStringField, writeObjectField, writeValue), direct `JsonWriter` instantiation, `JsonIo.toJson`/`toJava`/`formatJson`/`deepCopy`, `WriteOptions`/`ReadOptions` builders, prettyPrint emission, cyclic-graph round-trip. Same source code compiles and passes against both 4.102.0 and 4.103.0. The one notable forward-compatible improvement: POJO field emission in pretty-print mode now emits the post-colon space (`"name": "Alice"`) matching Jackson convention — 4.102.0 emitted `"name":"Alice"` (no space) which was inconsistent with @type field emission and a pre-existing bug. Code that asserts on exact pretty-print byte sequences may need updating but the semantic JSON contract is preserved.
* **CLEANUP**: **Deleted unused `getJsonGenerator()` package-private accessor + 2 stale `final Writer output = this.out;` locals.** `getJsonGenerator()` was added during the dog-food migration as a hook for internal callers to obtain `gen`; the migration is now complete and the accessor has zero callers (package-private, never escaped the package). The stale Javadoc on the `gen` field (which described `getJsonGenerator()` as a "public-facing accessor", incorrect since it was package-private and described as "external callers should not use this") was updated to reflect gen's actual role: owner of JsonWriter's structural emission, typed as concrete `CharStreamGenerator` for hot-path access to `writeXxxRaw` and `resetForBridge*` helpers. Two `final Writer output = this.out;` locals in `writeCollection` and `writeJsonObjectCollection` became unused after chunk 0b795b7f (writeSeparator) replaced their comma-emitting call sites with `gen.writeSeparator()`; removed. All 4092 tests pass; 0 failures.
* **TESTING**: **Tightened the last 3 `isEqualToIgnoringWhitespace` assertions to strict byte-equality / tree-equality.** `EnumTests.testEnumWithNoType_and_defaultWriter` and `testEnumWithPrivateMembersAsField_withPrivatesOff` now compare the writer's compact output byte-for-byte against the fixture file (`expected.trim()` to handle trailing-newline editor artifacts); the two fixture files (`default-enum-with-no-type.json`, `default-enum-standalone-without-privates.json`) were re-saved in compact form to match what `JsonWriter` produces with default options. `PrettyPrintTest.testPrettyPrint`'s pretty-vs-compact comparison was tightened from `isEqualToIgnoringWhitespace` to a gson tree-equality check (`JsonParser.parseString(json).isEqualTo(JsonParser.parseString(json1))`), which is stricter (catches numeric reformatting like `"1.5"` vs `"1.50"` that whitespace-stripping would silently accept). Zero `isEqualToIgnoringWhitespace` calls remain in the test suite. All 4092 tests pass; 0 failures.
* **FEATURE**: **`CharStreamGenerator.writeSeparator()` — fused `,` + newline+indent helper, replaces the 2-call `output.write(',') + gen.writeNewlineIndent()` pattern in JsonWriter's 9 element-loop separator sites.** Emits `','` followed by `\n` + indent at the current depth when prettyPrint is on, just `','` otherwise. State machine is NOT engaged — counterpart to `gen.writeNewlineIndent()` for callers that emit between-element separators manually. Eats more dog food (gen owns 100% of structural+separator emission for non-flat-pack element loops); the remaining 6 `output.write(',')` sites are in the flat-pack primitive-array handlers (`writeBooleanArray`, `writeDoubleArray`, etc.) where elements are emitted on a single line without indent, so a separator helper there would be slower (no `\n` + indent needed). Same-session 4-run vs 2-run A/B JsonPerformanceTest: raw deltas favored WITH writeSeparator but Read Ratio (which can't be affected by the change) showed the same ~3% drift, indicating environmental noise rather than a real perf effect — calibrated deltas are within noise floor. Committing the change for code-quality / architectural-purity reasons (more gen-driven), not for perf. All 4092 tests pass; 0 failures.
* **PERFORMANCE**: **`JsonWriter.writeImpl` wrapper's snap+restore replaced with narrow per-path snap+restore + trailing `gen.markValue()` — ~2-3% Write Ratio improvement.** Eliminated the previously load-bearing wrapper that bracketed every `writeImpl` call with a full `snapshotForExternalValue`/`restoreAfterExternalValue` arena copy. Two coordinated changes made the wrapper unnecessary:
  * **Body-method destructive resets removed**: 10 sites of `gen.resetForBridgeAtValueSlot(gen.currentDepth())` at the entry of `writePrimitiveArray`, `writeObjectArray`, `writeJsonObjectArray`, `writeCollection`, `writeJsonObjectCollection`, `writeJsonObjectMap`, `writeJsonObjectMapWithStringKeys`, `writeMap`, `writeMapWithStringKeys`, `writeJsonObjectObject` deleted. The reset's `contextStack[gen.depth] = FRAME_ROOT_EMPTY` write was destroying the outer caller's frame state — the wrapper's restore was un-destroying it. Without the reset, the outer frame stays intact across the body's push/pop, and `writeEndXxxRaw`'s `markValue()` at pop correctly transitions the outer state.
  * **3 still-load-bearing reset paths got their own narrow snap+restore**: `writeCustom` primitive-form dispatch (`useNewPrimitive`), `writeCustom` legacy-writer dispatch (`!useNewWrite`), and `writePrimitive`'s Long-wrap bare-value path (`!showType`) still legitimately reset gen state to emit a single value at root depth. Each now has its own `snapshotForExternalValue` + `restoreAfterExternalValue` pair AT the call site — only paying the arena round-trip cost when that specific path fires.
  * **Trailing `gen.markValue()` at end of public `writeImpl`**: covers the fast-path emissions (`null`, `Integer`/`Long`/`Boolean` raw writes, legacy primitive-form custom writers that emit via `out.write` rather than `gen`) that don't transition outer state on their own. For body-method paths that already `markValue` via `writeEndXxxRaw`, the trailing `markValue` is a no-op (FRAME_OBJECT_AFTER_VALUE and FRAME_ARRAY_AFTER_VALUE fall through `markValue` without effect).
  * **Comment cleanup** (chunk C): two `// When this.depth=0 ...` comments in `writeField` and `writePrimitiveFieldDirect` (referring to the now-eliminated field) updated to describe the actual invariant (state-machine transitions across the field-name → value path).
  * **2-run avg JsonPerformanceTest**: Write Ratio (cs=true) 1.88x → 1.83x (-2.7%), Write Ratio (cs=false) 1.705x → 1.675x (-1.8%), Read Ratio flat (1.835x → 1.845x within noise). All 4092 tests pass; 0 failures.
* **REFACTOR**: **`JsonWriter.this.depth` field + `tab()` private helper + 4 indentation-related private fields deleted; `newLine()` now delegates to `gen.writeNewlineIndent()` — chunk G of the `this.depth` elimination (completes task 32).** With all reads of `this.depth` migrated in chunks F-1..F-10, the field's mutations were dead writes — removed via bulk `replace_all`. Specifically deleted:
  * `private int depth = 0;` field declaration.
  * 13 × `final int outerDepth = this.depth;` captures and 13 × `this.depth = bodyDepth;` (or `gen.currentDepth();`) syncs and 11+ × `this.depth = outerDepth;` exit-restores across writeObjectArray, writeJsonObjectArray, writeCollection, writeJsonObjectCollection, writeJsonObjectObject, writeObject, writeMapToEnd (× 2 sections), writeMapBody (Iterator + JsonObject overloads), writeStringifiedMapBody, writeEnumSet.
  * The `tab(Writer output, int delta)` private method (formerly the indent emitter that read `this.depth`).
  * Private constants/fields that only `tab()` consumed: `NEW_LINE`, `maxIndentationDepth`, `indentationThreshold`, `indentationSize`. Also `json5UnquotedKeys` (orphan from earlier migrations) and `prettyPrint` (assigned-but-never-read — gen reads it via WriteOptions).
  * Stale comments referencing `this.depth` and the second load-bearing reason for the writeImpl wrapper (the `this.depth`/`gen.depth` divergence, which no longer exists).
  * Updated `writeImpl`'s method Javadoc — the wrapper now has only ONE load-bearing reason (body paths that intentionally reset gen state), not two. Removing the wrapper still requires inlining narrow snap+restore around those specific paths.
  * `newLine()` retained as a public delegating wrapper around `gen.writeNewlineIndent()` for any external custom writer that uses it. All internal callers migrated to call gen directly in chunk E.
  * Net diff: -47/+4 lines in JsonWriter.java. The dual-depth-tracking-system divergence is gone — gen's `depth` field is the single source of truth.
  * All 4092 tests pass; 0 failures.
* **REFACTOR**: **`writeEnumSet` bodyDepth migrated to `gen.currentDepth()` — chunk F-10.** Final method-level migration. `depthAtEntry + 2` math replaced with `gen.currentDepth()` captured after `writeStartArrayRaw`. `depthAtEntry` → `outerDepth`. **All READS of `this.depth` in JsonWriter's body-emission methods are now eliminated** — the remaining `this.depth` references are write-only mutations (`this.depth = bodyDepth` to sync for nested calls + `this.depth = outerDepth` to restore at exit) and two comment-only references. Next chunk: chunk G — delete the now-dead this.depth field + mutations + `newLine()` / `tab()` helpers. All 4092 tests pass; 0 failures.
* **REFACTOR**: **POJO emission pair (`writeObject` + `writeJsonObjectObject`) bodyDepth migrated to `gen.currentDepth()` — chunk F-9.** `writeJsonObjectObject`'s reset arg also updated. `writeObject` has no reset (already removed in chunk 6). Body-math assignment `this.depth = depthAtEntry + 1` replaced with `this.depth = gen.currentDepth()` (captured AFTER `writeStartObjectRaw`). `depthAtEntry` → `outerDepth` for exit restore semantic. All 4092 tests pass; 0 failures.
* **REFACTOR**: **String-key map family reset args + bodyDepth migrated to `gen.currentDepth()` — chunk F-8.** Five methods migrated together since they share the `depthAtEntry+1` body-math pattern: `writeMapWithStringKeys` + `writeJsonObjectMapWithStringKeys` (reset args), `writeMapBody` (Iterator overload) + `writeMapBody` (JsonObject overload) + `writeStringifiedMapBody` (each captures `bodyDepth = gen.currentDepth()` at entry — the caller has already opened the outer object so gen.depth is at the body level). All 4092 tests pass; 0 failures.
* **REFACTOR**: **`writeJsonObjectMap` reset arg migrated to `gen.currentDepth()` — chunk F-7.** Trivial one-line change: `writeJsonObjectMap` delegates body emission to `writeMapToEnd` (already migrated in chunk F-6), so only its own reset arg needs updating. The body math is gen-anchored regardless of which caller (writeMap or writeJsonObjectMap) invoked writeMapToEnd. All 4092 tests pass; 0 failures.
* **REFACTOR**: **`writeMap` + `writeMapToEnd` reset arg + bodyDepth migrated to `gen.currentDepth()` — chunk F-6.** Pair migrated together since `writeMap` calls `writeMapToEnd` and they share depth state across the call boundary. `writeMap`'s reset arg → `gen.currentDepth()`. `writeMapToEnd`'s `bodyDepth = depthAtEntry + 2` math replaced with `bodyDepth = gen.currentDepth()` captured after the first `writeStartArrayRaw` for the @keys body. The captured value is reused for the @items section (the pop+push pair preserves the same depth value). `outerDepth` saves the OUTER this.depth for the exit-restores at the end of each array section. Note: `writeMapToEnd` is also called from `writeJsonObjectMap` which is NOT yet migrated — the mixed scenario works because `writeMapToEnd`'s gen-anchored bodyDepth correctly tracks whichever caller's reset behavior is in effect. All 4092 tests pass; 0 failures.
* **REFACTOR**: **`writeJsonObjectCollection` reset arg + bodyDepth migrated to `gen.currentDepth()` — chunk F-5.** Completes the 4 array-shaped emitters (writeObjectArray, writeJsonObjectArray, writeCollection, writeJsonObjectCollection). All 4092 tests pass; 0 failures.
* **REFACTOR**: **`writeCollection` reset arg + bodyDepth migrated to `gen.currentDepth()` — chunk F-4.** Same pattern as F-2/F-3 applied to Collection emission. All 4092 tests pass; 0 failures.
* **REFACTOR**: **`writeJsonObjectArray` reset arg + bodyDepth migrated to `gen.currentDepth()` — chunk F-3.** Same pattern as chunk F-2 applied to the JsonObject overload of array emission. Reset arg → `gen.currentDepth()`; `bodyDepth = gen.currentDepth()` captured after `writeStartArrayRaw`; `depthAtEntry` local renamed to `outerDepth` for the exit-restore semantic. All 4092 tests pass; 0 failures.
* **REFACTOR**: **`writeObjectArray` reset arg + bodyDepth capture migrated to `gen.currentDepth()` — chunk F-2.** Both changes must happen together: the OLD reset force-anchored gen.depth to this.depth, so the OLD `bodyDepth = this.depth + N` math matched gen's actual depth. With the new gen-anchored reset, gen.depth is NOT force-changed, so the body depth comes from gen.depth after the pushes (= `gen.currentDepth()` captured after `writeStartArrayRaw` / `writeStartObjectRaw`). Renamed the exit-restore local from `depthAtEntry` to `outerDepth` to reflect its new semantic: it saves the OUTER caller's `this.depth` (so the field is properly save/restored across the method), distinct from the gen-derived `bodyDepth` used inside the body. `this.depth = bodyDepth` mutation stays — preserves the contract for any not-yet-migrated nested method whose reset still reads `this.depth`. Behavior change: in nested-from-legacy-custom-writer pretty-print, an Object[] value emits at its actual structural depth (proper nesting) instead of `this.depth+N` (artificially low). Compact output unchanged. All 4092 tests pass; 0 failures.
* **REFACTOR**: **`writePrimitiveArray`'s `resetForBridgeAtValueSlot` argument migrated to `gen.currentDepth()` — chunk F-1 of the `this.depth` field elimination.** Re-adds the `int currentDepth()` package-private accessor on `CharStreamGenerator` (it was added then reverted during a failed chunk-B attempt). The reset's depth argument now sources from gen's own structural depth rather than the legacy `this.depth` tracker, so body emission lands at the structurally-correct depth in nested-from-legacy-custom-writer cases where the two diverged. Smallest possible single-method validation of the migration approach: `writePrimitiveArray` has no body-depth math (its primitive handlers don't call any methods that reset gen state) so the only this.depth touchpoint is the entry reset. **Behavior change**: in nested-from-legacy-custom-writer pretty-print cases, the primitive array's leading body indent is now at the actual structural depth (proper nesting) instead of `this.depth` (artificially low). Compact output is unchanged. All 4092 tests pass; 0 failures. Subsequent chunks migrate the remaining 9 body-emitting methods.
* **REFACTOR**: **Element-loop `newLine()` calls migrated to `gen.writeNewlineIndent()` — chunk E of the `this.depth` field elimination.** 8 sites in `writeObjectArray`, `writeCollection` (× 2), `writeJsonObjectArray`, `writeJsonObjectCollection`, `writeMapToEnd` (× 2 — @keys + @items), and `writeEnumSet` swapped `output.write(',') + newLine()` for `output.write(',') + gen.writeNewlineIndent()`. Behavior-preserving — at each call site, `gen.depth == this.depth == bodyDepth`, so the gen-driven indent emission reads the same depth as the legacy `newLine()` path. `gen.writeNewlineIndent()` is a no-op in compact mode, same as `newLine()`. The 9th remaining `newLine()` call in `writeCustom`'s legacy-API dispatch (line 815) is NOT migrated in this chunk — its depth context differs (`this.depth` is the outer caller's depth, not the body's), and the migration there requires reworking that dispatch's depth handling separately. Same-session 3-run JsonPerformanceTest: Write Ratio (cs=true) median 1.88x (avg 1.92x, runs 2.05/1.88/1.83) vs prior chunk's 1.84x — within the ~8-10% run-to-run noise envelope. All 4092 tests pass; 0 failures.
* **REFACTOR**: **Map-family element-loop `restoreDepth(this.depth)` calls migrated to use `bodyDepth` locals — chunk D of the `this.depth` field elimination.** Same pattern as chunks B/C: `writeMapToEnd` (4 sites — @keys array + @items array share a single `bodyDepth = depthAtEntry + 2` since both bodies sit at the same depth), `writeMapBody` Iterator overload (1 site), `writeMapBody` JsonObject overload (1 site), `writeStringifiedMapBody` (1 site). Total 7 `gen.restoreDepthAfterExternalValue(this.depth)` sites replaced with `gen.restoreDepthAfterExternalValue(bodyDepth)`. **All `restoreDepth(this.depth)` reads in JsonWriter are now eliminated.** The next chunks tackle the remaining `this.depth` reads (`newLine()` indent emission and the entry `resetForBridgeAtValueSlot(this.depth)` call) before the field + mutations can be deleted. All 4092 tests pass; 0 failures.
* **REFACTOR**: **`writeCollection` / `writeJsonObjectArray` / `writeJsonObjectCollection` element-loop `restoreDepth(this.depth)` calls migrated to use `bodyDepth` locals.** Same pattern as the chunk-B `writeObjectArray` migration — 7 additional `gen.restoreDepthAfterExternalValue(this.depth)` sites swapped for `gen.restoreDepthAfterExternalValue(bodyDepth)`, where `bodyDepth = depthAtEntry + (wrapped ? 2 : 1)` is captured as a `final int` local right after the structural opens. The `this.depth = bodyDepth` mutation stays for now (still needed for nested calls). Total `restoreDepth(this.depth)` reads remaining: 7 (all in the map family — `writeMap`, `writeJsonObjectMap`, `writeMapBody` × 2, `writeStringifiedMapBody`). All 4092 tests pass; 0 failures.
* **REFACTOR**: **`writeObjectArray`'s element-loop `restoreDepth(this.depth)` migrated to use a `bodyDepth` local.** First chunk of the `this.depth` field elimination (task 32 of the JsonWriter dog-food migration). The body-depth math (`depthAtEntry + (wrapped ? 2 : 1)`) is now captured into a `final int bodyDepth` local, and `gen.restoreDepthAfterExternalValue(this.depth)` inside the element loop is replaced with `gen.restoreDepthAfterExternalValue(bodyDepth)`. Behavior-preserving substitution — `bodyDepth` is computed from the same expression that `this.depth` was assigned to. The `this.depth = bodyDepth` mutation stays in place for now (still needed for nested writeImpl calls that read `this.depth` for their own `gen.resetForBridgeAtValueSlot` argument and `newLine()` indent). Subsequent chunks will migrate the remaining methods, then `newLine()` → `gen.writeNewlineIndent()`, then the reset, then delete the field. All 4092 tests pass; 0 failures.
* **FEATURE**: **`CharStreamGenerator.writeNewlineIndent()` — public helper that emits `\n` + indent at the current `gen.depth` when prettyPrint is on (no-op otherwise).** Counterpart to `JsonWriter.newLine()`, intended for callers that emit their own `,` separator between elements (legacy JsonWriter element-loop pattern) and need the matching newline+indent that the state machine would otherwise produce via `emitIndentIfPretty()`. The indent level comes from gen's own `depth` field — callers don't need to track depth separately. First step of chunk-A in the JsonWriter `this.depth` / `newLine()` / `tab()` elimination — no JsonWriter changes yet, just the gen-side primitive that subsequent chunks will migrate callers onto. Two new `JsonGeneratorTest` cases verify depth-scaled indent emission and the compact-mode no-op behavior. All 4092 tests pass; 0 failures.
* **REFACTOR**: **Ninth chunk of the writeImpl structural-emission migration — `writeCustom`, `writeImplInternal`'s @IoValue path, and `writeEnumSet` migrated; `writeId` / `writeType` / `tabIn` / `tabOut` deleted.** Same pattern as the prior chunks: the structural `{`/`}` envelope, `@id` / `@type` emission, and `@items` array bodies all go through gen's Raw helpers. With these three callers migrated, `writeId(int)` and `writeType(String)` have no remaining callers and are deleted (-30 LOC).
  * **`writeCustom`**: The object-form custom-writer dispatch now opens `{}` via `gen.writeStartObjectRaw` (replacing `out.write('{')` + `tabIn`), emits `@id`/`@type` via `gen.writeNumberField(idKey, ...)` / `gen.writeStringFieldUnescaped(typeKey, ...)` directly (replacing `writeId`/`writeType`), closes via `gen.writeEndObjectRaw` (replacing `tabOut` + `out.write('}')`). The legacy writer-dispatch path retains a small reset for backward compat: a new `gen.resetCurrentObjectFrame()` helper flips the CURRENT top frame back to `FRAME_OBJECT_EMPTY` (no depth change) so legacy writers that call back via `context.writeFieldName` (which delegates to gen) don't auto-emit a second leading comma after JsonWriter's manual `","` prelude. The new-API writer path (writers that take `JsonGenerator` directly) gets no reset — gen state is `FRAME_OBJECT_AFTER_VALUE` after `@id`/`@type`, and gen's `writeFieldName` auto-emits the leading comma correctly. The primitive-form dispatch retains the prior `resetForBridgeAtValueSlot` (rare case, preserves custom-writer compat — same reasoning as the chunk-8 Long-wrap bare-value path).
  * **`@IoValue` path** (in `writeImplInternal`): Replaces `out.write('{')` + `tabIn` + `writeType` + `out.write(',') + newLine + writeBasicString("value") + out.write(':') + writeImpl + tabOut + newLine + out.write('}')` with `gen.writeStartObjectRaw + gen.writeStringFieldUnescaped(typeKey, alias) + gen.writeFieldName("value") + writeImpl + gen.writeEndObjectRaw`. -8 LOC.
  * **`writeEnumSet`**: Major restructure. Outer `{}` via Raw helpers. `@id` via `gen.writeNumberField(idKey, ...)`. `@type` (or `@enum` for the old-way option) via `gen.writeStringField`. `@items` field + array opening via `gen.writeFieldName(ITEMS) + gen.writeStartArrayRaw + gen.beginInlineArrayBody`. Per-element loop preserves the simple-enum (writeStringValue) vs complex-enum (writeStartObjectRaw + writeField loop + writeEndObjectRaw) dispatch — the complex-enum block no longer needs the snapshot/restore wrap since `writeStartObjectRaw` doesn't destroy outer state. Closes with `gen.writeEndArrayRaw + gen.writeEndObjectRaw`. Empty EnumSet emits `{"@type":"...","@items":[]}` correctly via the empty-array short-circuit.
  * **Deletions**: `writeId(int)`, `writeType(String)`, `tabIn()`, `tabOut()` — all unused after this chunk. `newLine()` retained (still used by the migrated element-loop emissions for between-element separators).
  * All 4090 tests pass; 0 failures. Same-session avg-of-2 JsonPerformanceTest: Write Ratio 1.84x — flat vs the prior chunk's 1.84x.
* **REFACTOR**: **Eighth chunk of the writeImpl structural-emission migration — `writeOptionalReference` `@ref` path and `writePrimitive` Long-wrap wrapped path drop the `resetForBridgeAtValueSlot`.** Same pattern as the chunk-6 `writeObject` refactor: `writeStartObjectRaw` pushes from the current `gen.depth` without overwriting the outer caller's frame state, and `writeEndObjectRaw`'s pop + markValue transitions the outer frame correctly on exit. The reset is no longer needed.
  * **`writeOptionalReference`**: `{"@ref":<id>}` emission. Was `gen.resetForBridgeAtValueSlot() + writeStartObject + writeNumberField + writeEndObject`. Now `writeStartObjectRaw + writeNumberField + writeEndObjectRaw`.
  * **`writePrimitive` Long-wrap (showType=true)**: `{"@type":"long","value":"..."}` wrapped emission. Was `gen.resetForBridgeAtValueSlot()` outside the if/else, then `writeStartObject + writeStringFieldUnescaped + longBoxedWriter.write + writeEndObject`. Now uses `writeStartObjectRaw + ... + writeEndObjectRaw` with no leading reset.
  * **`writePrimitive` Long-wrap (showType=false)** — the bare `"<long-as-string>"` value path — **retains the reset**. `longBoxedWriter.write(obj, false, gen, ctx)` routes through `PrimitiveTypeWriter.write` → `LongWriter.writePrimitiveForm` → `gen.writeString` which engages the state machine. If the caller (e.g., writeJsonObjectArray's element loop) is in `FRAME_ARRAY_AFTER_VALUE` with its own manual `,\n + indent` separator between elements, the gen-driven emission would add a SECOND comma. The reset puts gen at `FRAME_ROOT_EMPTY` so no separator is emitted. Caller restores depth afterward. Keeping the reset for this rare path (only fires under `writeLongsAsStrings=true` AND Long element AND `!forceType`) preserves custom-writer compatibility for user-registered LongWriter overrides.
  * **Caller cleanup**: `writeObjectArray`'s Long-element case removed the now-wasted `gen.restoreDepthAfterExternalValue` (since writeObjectArray passes `writeLongsAsStrings` as showType, the Long-wrap fires only with showType=true → wrapped path → no reset; the bare-value path never fires there). `writeJsonObjectArray`'s Long/Boolean/Double case changed from `if (forceType)` conditional restore to unconditional restore — fixes a latent bug for the bare-value Long path when `writeLongsAsStrings=true` AND `!forceType` (the only path where a reset actually still fires); no-op overhead for other cases is ~1ns.
  * All 4090 tests pass; 0 failures. Same-session avg-of-2 JsonPerformanceTest: Write Ratio 1.84x — flat vs the prior chunk's 1.835x (within run-to-run noise envelope).
* **REFACTOR**: **Seventh chunk of the writeImpl structural-emission migration — `writeStringValue` converted to state-machine-free direct emission.** The previous implementation did `gen.resetForBridgeAtValueSlot()` + `gen.writeString(s)` — a "scratch use" of gen that destroyed `contextStack[0]` and went through the state machine's startValueContext + markValue pair. The new implementation calls `CharStreamGenerator.writeJsonUtf8String` (or `writeSingleQuotedString` under json5SmartQuotes) directly to the underlying writer — no state machine engagement at all. Caller is responsible for any necessary state transition (callers in object-body context — writeField, writeMapBody — already call `gen.markValue()` after writeStringValue to flip the outer frame from `FRAME_OBJECT_AFTER_FIELD` to `FRAME_OBJECT_AFTER_VALUE`). Same design philosophy as the `writeXxxRaw` family for scalars: structural emission engages the state machine, value emission inside element loops skips it.
  * **Caller cleanups**: 4 sites in `writeObjectArray` (the String element fast path) and `writeJsonObjectArray` (the char/String fast paths) had `gen.restoreDepthAfterExternalValue(this.depth)` calls immediately after writeStringValue — those are now no-ops (writeStringValue no longer touches gen.depth) and were removed for clarity.
  * **Dead code removed**: `writeElements(Writer, Iterator)` helper (no remaining callers after the chunk-4 Map migration inlined the loop); JsonWriter's private `isValidJson5Identifier` / `isIdentifierStart` / `isIdentifierPart` helpers (no callers — `CharStreamGenerator` has its own copy used by `writeKey`, and `WriteOptionsBuilder` has its own copy for `serializedKey` precomputation).
  * All 4090 tests pass; 0 failures. Same-session avg-of-2 JsonPerformanceTest: Write Ratio 1.835x — flat vs the prior chunk's 1.825x (within run-to-run noise envelope; writeStringValue migration saves the state-machine work but the perf delta is in the noise floor).
* **REFACTOR**: **Sixth chunk of the writeImpl structural-emission migration — string-key Map paths (`writeMapWithStringKeys`, `writeJsonObjectMapWithStringKeys`, `writeMapBody` × 2 overloads, `writeStringifiedMapBody`) migrated to drive structural emission through `CharStreamGenerator`.** Same pattern as the prior chunks: outer `{`/`}` via Raw helpers, `@id`/`@type` fields via gen field helpers, per-entry key via `gen.writeFieldName` (handles json5UnquotedKeys + escape + auto-comma/indent), per-entry value via `writeCollectionElement` followed by lightweight `gen.restoreDepthAfterExternalValue(this.depth)` + `gen.markValue()`. Both `writeMapBody` overloads (Iterator and JsonObject) collapse from ~70 lines × 3 branches each to a single uniform loop — gen handles the json5/escape/separator/indent permutations internally.
  * **Helpers deleted**: `emitIdAndTypeIfNeeded(JsonObject, boolean, Writer)` and `writeIdAndTypeIfNeeded(Object, boolean, boolean)` are now unreachable after all their callers (writeJsonObjectMapWithStringKeys, writeMapWithStringKeys) inlined the @id/@type emission. Deletion saves -30 LOC.
  * **Critical refactor — `writeObject` no longer resets gen state**: the bodyOnly=false path previously called `gen.resetForBridgeAtValueSlot(this.depth)` before `gen.writeStartObjectRaw()`. That reset OVERWROTE `contextStack[this.depth]` (the outer caller's frame state), which was destructive for callers like `writeCollectionElement`'s POJO short-circuit (where the outer caller is `writeMapBody` with state `FRAME_OBJECT_AFTER_FIELD` set by a preceding `writeFieldName`). Fix: drop the reset entirely — `writeStartObjectRaw` pushes from the current `gen.depth` without overwriting the outer frame. On exit, `writeEndObjectRaw`'s pop + markValue transitions the outer frame from `FRAME_OBJECT_AFTER_FIELD` to `FRAME_OBJECT_AFTER_VALUE` correctly. For the wrapper-protected path (writeImpl wrapper), the wrapper's snap+restore+markValue still produces the same outer-state transition. This eliminates the need for caller-side snapshot/restore around the POJO short-circuit, which was a workaround attempted in this chunk.
  * All 4090 tests pass; 0 failures. Same-session avg-of-2 JsonPerformanceTest: Write Ratio 1.825x — actually **~2.5% better** than the prior chunk's 1.87x, because eliminating the writeObject reset removes one `resetForBridgeAtValueSlot` call per POJO emission (a frequent path in the perf test data).
* **REFACTOR**: **Fifth chunk of the writeImpl structural-emission migration — `writeJsonObjectObject` and `writeObject` (POJO body emission) migrated to drive structural emission through `CharStreamGenerator`.** The outer `{`/`}`, `@id`/`@type` fields, and per-field emission now all go through gen. `writeField`'s API simplified: the `first` flag parameter is dropped — gen's state machine auto-handles leading separator (comma + indent for subsequent fields, indent-only for first field). `writeField`'s per-field flow: `gen.writeFieldNameRaw(plan.serializedKey())` for the precomputed key emission, then either a gen-driven value method (`gen.writeString` for String, `gen.writeNull` for null) OR a state-machine-free raw helper (`SMALL_INT_STRINGS`/`writeIntRaw`/`writeLongRaw`/`writePrimitive`) followed by `gen.markValue()` to transition the frame from `FRAME_OBJECT_AFTER_FIELD` to `FRAME_OBJECT_AFTER_VALUE`. `writeImpl`-path values (the slow-path complex types) are handled by the writeImpl wrapper's existing snapshot/restore. `writeAnyGetterFields` similarly drops its `first` parameter. `writePrimitiveFieldDirect` gains a trailing `gen.markValue()` for all primitive kinds (and uses `gen.writeString` directly for `PRIMITIVE_CHAR` instead of `writeStringValue`).
  * **Critical depth=0 fix**: when `this.depth=0` (e.g., inside `writeEnumSet`'s enum-as-object block where the object body lives at `contextStack[0]` itself), `writeStringValue`'s `resetForBridgeAtValueSlot()` would OVERWRITE `contextStack[0]` from `FRAME_OBJECT_AFTER_FIELD` to `FRAME_ROOT_DONE`, destroying the object context. The next field's `writeFieldNameRaw` then sees an invalid state and throws. Fix: in the migrated `writeField` and `writePrimitiveFieldDirect`, bypass `writeStringValue` (which resets gen) and call `gen.writeString` directly — it handles state correctly from `FRAME_OBJECT_AFTER_FIELD`. The standalone `writeStringValue` helper is retained for legacy callers that emit at non-`depth=0` positions.
  * **`writeEnumSet`'s enum-as-object block** updated to call the new `writeField(obj, plan)` signature (no `first`), bracketed with `snapshotForExternalValue` + `resetForBridgeInsideObjectBody(this.depth)` + writeField loop + `restoreAfterExternalValue` so gen state is reset for the inner field block and restored on exit (the surrounding `writeEnumSet` legacy code uses `out.write` for the `[`/`]` brackets so the snap+restore isolates the gen-driven inner work).
  * All 4090 tests pass; 0 failures. Same-session avg-of-2 JsonPerformanceTest: Write Ratio 1.87x (vs prior chunk's 1.805x — ~3.5% above the prior measurement, at the upper edge of run-to-run noise; POJO emission is the dominant code path in the perf test so per-field gen-state-machine overhead accumulates).
* **REFACTOR**: **Fourth chunk of the writeImpl structural-emission migration — `writeMap`, `writeJsonObjectMap`, and `writeMapToEnd` migrated to drive structural emission through `CharStreamGenerator`.** The {`@type` / `@id` / `@keys` / `@items`}-field envelope, the two array bodies (one for keys, one for values), and the closing `}` all go through gen's Raw structural-token family + `writeFieldNameRaw` + `beginInlineArrayBody`. The per-element loops for both arrays use the existing `writeCollectionElement` dispatch with lightweight `gen.restoreDepthAfterExternalValue(this.depth)` after each call — same pattern as the array/collection migrations. `this.depth` is temporarily synced to the body depth (`entry + 2` for the array-body inside the object-body) across each array's element loop so legacy `newLine()` indents at the correct level. `writeJsonObjectMap` no longer routes through the legacy `emitIdAndTypeIfNeeded` helper — the @id/@type emission is inlined via `gen.writeNumberField(idKey, jObj.id)` + `gen.writeStringFieldUnescaped(typeKey, alias)` directly. The "type silently skipped when getTypeNameForOutput returns null" legacy behavior is preserved. `emitIdAndTypeIfNeeded` is retained for the still-legacy `writeJsonObjectMapWithStringKeys` path. **Bonus micro-optimization**: `writeCollection`'s element loops and `writeMapToEnd`'s @keys/@items element loops switched from the per-iteration "is this the first element?" check (`if (idx > 0)` / `if (wroteElement)`) to the peel-off-first pattern — emit the first element + restoreDepth outside the loop, then the loop emits `,\n + indent` BEFORE each subsequent element. Removes one branch per iteration. All 4090 tests pass; 0 failures. Same-session avg-of-2 JsonPerformanceTest: Write Ratio 1.805x (vs prior chunk's 1.86x — ~3% better, likely the peel-off optimization plus run-to-run noise overlap).
* **REFACTOR**: **Third chunk of the writeImpl structural-emission migration — `writeCollection`, `writeJsonObjectCollection`, and `writeJsonObjectArray` migrated to drive structural emission through `CharStreamGenerator`.** Same pattern as the previous `writePrimitiveArray` / `writeObjectArray` migrations: `gen.resetForBridgeAtValueSlot(this.depth)` at entry; `writeStartObjectRaw` / `writeEndObjectRaw` / `writeStartArrayRaw` / `writeEndArrayRaw` / `writeFieldNameRaw(itemsPrefix)` / `beginInlineArrayBody` for structural tokens; lightweight `gen.restoreDepthAfterExternalValue(this.depth)` after each `writeCollectionElement` / `writeArrayElementIfMatching` / `writePrimitive(forceType)` / `writeStringValue` call (since those paths internally reset gen.depth=0 via the no-arg `resetForBridgeAtValueSlot()`). `this.depth` is temporarily synced to the array body depth so the legacy `newLine()` emissions between elements indent at the correct level. **`writeCollection` preserves an asymmetric behavior from the legacy code**: an empty wrapped Collection emits `{"@type":"...","@id":N}` with NO trailing `"@items":[]` field — differs from `writePrimitiveArray` / `writeObjectArray` which DO emit `"@items":[]` for the empty case. **`writeJsonObjectCollection` preserves the legacy "always wrap on empty" semantic** — an empty JsonObject collection emits `{}` even when neither `showType` nor `referenced` is set (legacy code unconditionally opens `{` when `len==0`). **`beginCollection(showType, referenced)` helper is deleted** — both of its callers (writeCollection, writeJsonObjectCollection) inlined the emission. All 4090 tests pass; 0 failures. Same-session JsonPerformanceTest median-of-3: Write Ratio JsonIo/Jackson 1.81x (vs same-session baseline 1.87x at the pre-migration commit — perf flat-to-slightly-better within run-to-run noise). The lightweight depth-only restore is what keeps the per-element cost in the noise envelope; a full `snapshotForExternalValue` / `restoreAfterExternalValue` per element would have been ~25ns × elements-per-test = real regression at scale.
* **REFACTOR**: **Second chunk of the writeImpl structural-emission migration — `writeObjectArray` migrated to drive structural emission through `CharStreamGenerator`.** The `{`/`}` wrap envelope (when `@type` / `@id` is emitted), the `[`/`]` brackets, and the `@items` prefix all go through `gen.writeStartObjectRaw` / `writeEndObjectRaw` / `writeStartArrayRaw` / `writeEndArrayRaw` / `writeFieldNameRaw` / `beginInlineArrayBody`. The per-element loop keeps the legacy specialized fast paths (`writePrimitive`, `writeStringValue`, `writeArrayElementIfMatching`, `writeImpl`) — these are bracketed by a lightweight `gen.restoreDepthAfterExternalValue(this.depth)` when the path internally calls `gen.resetForBridgeAtValueSlot()` (writeStringValue, writePrimitive's Long-wrap branch, writeCustom's new-API dispatch from writeArrayElementIfMatching). The full snapshot/restore arena (`snapshotForExternalValue` / `restoreAfterExternalValue`) is too heavy for per-element use (~25ns/pair × N elements = noticeable regression at scale); the new depth-only restore is ~2ns and works because all those reset paths operate at gen.depth=0 and don't touch the array body's `contextStack[body_depth]` — only the depth field needs restoring. Two supporting fixes shipped alongside: (a) **`CharStreamGenerator.resetForBridgeAtValueSlot(int depth)` no longer sets `suppressNextIndent=true`** — that was incorrectly suppressing the indent INSIDE the body (between `{` and the first field key) rather than the leading indent before `{`. Since the migrated callers use `writeStart*Raw` (which skip `startValueContext`), there's no leading `emitIndentIfPretty` to suppress in the first place; the suppression was killing the legitimate body indent. (b) **`writeArrayElementIfMatching` path's gen state preservation** — this call dispatches to `writeCustom` which resets gen.depth=0 for the new-API custom-writer path, but `writeArrayElementIfMatching` doesn't go through `writeImpl`'s wrapper, so the array body's gen state was being destroyed mid-loop. The new lightweight depth restore fixes it. `this.depth` is temporarily synced to the array body depth so legacy `newLine()` emissions between elements indent at the correct level. All 4090 tests pass; 0 failures. Median-of-5 JsonPerformanceTest: 1.78x JsonIo/Jackson Write Ratio — flat vs the 1.79x pre-migration baseline (well within run-to-run noise envelope).
* **PERFORMANCE**: Recover the ~4% Write Ratio regression introduced by the writePrimitiveArray migration. Three changes combine to restore perf to the pre-migration baseline (median 1.79x JsonIo/Jackson Write Ratio, matching the historical 1.75-1.81x envelope): (a) **Raw structural-token family on `CharStreamGenerator`** — `writeStartObjectRaw` / `writeStartArrayRaw` / `writeEndObjectRaw` / `writeEndArrayRaw` package-private fast paths that bypass the `startValueContext` switch + the validation throw on end methods. State tracking (push/pop, outer-frame `markValue` after pop) preserved so subsequent gen calls remain correct. Counterpart to the `writeXxxRaw` scalar family for structural tokens, exactly as the user's intuition suggested. (b) **Reset moved from the writeImpl wrapper into the migrated method** — `gen.resetForBridgeAtValueSlot(this.depth)` now runs inside `writePrimitiveArray` rather than in `JsonWriter.writeImpl`'s wrapper. Result: non-migrated dispatch paths (the overwhelming majority of writeImpl calls — primitive scalars, POJO emission via writeObject, collections, maps, etc.) no longer pay the per-call reset overhead. Only the migrated paths that need a clean gen state at entry pay for the reset. (c) **`writeStringValue` reverted to the cheap form** (no snapshot/restore wrap) by special-casing char[] in writePrimitiveArray to use `gen.writeString` directly at `FRAME_ARRAY_EMPTY` rather than going through the `writeStringValue → writeStringValue-from-FRAME_ARRAY_AFTER_VALUE-needs-wrap` path. The PRIM_ARRAY_WRITERS entry for char[] is now dead (left as a documenting comment). All 4090 tests pass; 0 failures. Median-of-7 perf: 1.77/1.78/1.79/1.79/1.80/1.84/1.86 — clearly within the pre-migration noise envelope.
* **REFACTOR**: **First chunk of the writeImpl structural-emission migration — `writePrimitiveArray` migrated to drive ALL structural emission through `CharStreamGenerator`.** Replaces the legacy `output.write('{')` / `output.write('[')` / `output.write(',')` / `tabIn` / `tabOut` / `newLine` / `output.write(itemsPrefix)` punctuation in `writePrimitiveArray` with `gen.writeStartObject()` / `writeEndObject()` / `writeStartArray()` / `writeEndArray()` / `writeFieldNameRaw(itemsPrefix)` / `beginInlineArrayBody()` calls — gen's structural state machine drives the entire envelope, the prim-array-handler inner loop (the `1,2,3` flat-pack body) stays state-machine-free via the existing `writeXxxRaw` helpers + manual commas. Two new package-private hooks on `CharStreamGenerator` ship to enable the migration: (a) `beginInlineArrayBody()` — opens an array body for raw flat-pack emission, emits the leading indent that gen would normally produce on the first element, and flips state to `FRAME_ARRAY_AFTER_VALUE` so `writeEndArray` emits the correct trailing indent; (b) `resetForBridgeAtValueSlot(int depth)` — depth-aware variant of the existing `resetForBridgeAtValueSlot()` that aligns gen's depth tracker with `JsonWriter.this.depth` so pretty-print indent emission inside the body matches the actual document depth (the outer object was opened via legacy `out.write('{')` + `tabIn` that gen wasn't driving). `JsonWriter.writeImpl` wrapper updated to use the depth-aware variant — captures outer state via `snapshotForExternalValue`, resets gen to a clean value-slot at the correct depth (with `suppressNextIndent=true` so the first emit doesn't add a spurious leading newline at the legacy/gen-driven seam), runs `writeImplInternal`, then `restoreAfterExternalValue` restores outer state. `JsonWriter.writeStringValue` wrapped with the same `snapshotForExternalValue` / `restoreAfterExternalValue` pattern so its internal `resetForBridgeAtValueSlot` (needed because writeStringValue is called from a value slot in many places) no longer destroys the outer gen state — required because the migrated `writePrimitiveArray` is now in `FRAME_ARRAY_AFTER_VALUE` when its char[] handler calls `writeStringValue` to emit the entire char array as a single string element. **Behavior change**: `writeFieldNameRaw` now auto-emits the leading separator (comma between fields, indent in pretty-print) to match the public `writeFieldName` semantics — was previously a no-separator "caller emits comma manually" contract that no internal caller actually used (only the test exercised it). Net result: writePrimitiveArray's pretty-print output matches legacy byte-for-byte across all primitive array types (boolean, byte, char, short, int, long, float, double) — verified across wrapped (`{"@type":...,"@items":[...]}`), unwrapped (`[...]`), empty, and POJO-field-nested cases. All 4088 tests pass; 0 failures. Same-session JsonPerformanceTest single-run: Write Ratio JsonIo/Jackson 1.93x (cycleSupport=true) — within run-to-run noise envelope of baseline. 4 new tests in `JsonGeneratorTest` cover the new gen hooks (`writeFieldNameRaw_emitsIndentInPrettyPrint`, `beginInlineArrayBody_flatPacksValuesWithCorrectTrailingIndent`, `beginInlineArrayBody_compactModeOmitsIndents`, `beginInlineArrayBody_rejectsOutOfArrayContext`).
* **FEATURE**: Added `CharStreamGenerator.writeFieldNameRaw(String preformattedKeyAndColon)` — package-private fast-path hook for emitting key+colon pairs that the caller has already formatted (the precomputed-prefix pattern used by JsonWriter for `@items` / `@keys` meta-key emission). Skips the per-call quote-decision + escape-scan that `writeFieldName(String)` performs. Transitions state from `FRAME_OBJECT_EMPTY` / `FRAME_OBJECT_AFTER_VALUE` to `FRAME_OBJECT_AFTER_FIELD`. Same pattern as `writeStringFieldUnescaped` (state-machine-aware fast path for known-safe input), now applied to the field-name side. Foundation for the `writeImpl` structural-emission migration. 1 new test in `JsonGeneratorTest` covers the round-trip. All 4084 tests pass; 0 failures.
* **REFACTOR**: **Step 3 of the dog-food migration — full Jackson API delegation on `JsonWriter`.** All 13 of JsonWriter's public Jackson-style API methods (`writeStartObject` / `writeEndObject` / `writeStartArray` / `writeEndArray` / `writeFieldName` / `writeStringField` / `writeNumberField(Number)` / `writeBooleanField` / `writeObjectField` / `writeArrayFieldStart` / `writeObjectFieldStart` / `writeValue(String)` / `writeValue(Object)`) now delegate to `CharStreamGenerator`. JsonWriter's parallel state machine — the `Deque<WriteContext> contextStack`, the `WriteContext` enum, the `writeCommaIfNeeded` / `markObjectFieldWritten` / `markArrayElementWritten` helpers, and the `writeKey` helper — is **eliminated**. gen is the single source of truth for structural state. **The most substantial dog-food milestone since D1.** Key enablers landed in the same commit: (a) new package-private `gen.snapshotForExternalValue()` / `restoreAfterExternalValue(int)` hooks that capture the FULL `contextStack[0..depth]` (not just the top byte) into a reusable side-buffer arena (no per-call allocation after initial sizing); (b) `JsonWriter.writeImpl` now wraps the internal serialization body (`writeImplInternal`) in a snapshot/restore window so gen's state is reliably restored after the (potentially deeply recursive) writeImpl returns — this is what makes custom writers like `MultiKeyMapWriter`, `CompactMapWriter`, `CompactSetWriter` continue to work even though their write methods mix `context.writeImpl(...)` recursion with `context.writeFieldName / writeObjectField` Jackson API calls. (c) `startValueContext` and `markValue` were made package-private on gen so `writeValue(Object)` can emit the leading separator before delegating to writeImpl. (d) `writeCustom` now resets gen state via `resetForBridgeInsideObjectBody` for BOTH the new-API and legacy-API dispatch paths — the legacy path also needs it now that the Jackson API methods route through gen. **Behavior change**: the legacy "writeStringField always emits a LEADING comma" semantics is REMOVED — gen auto-decides comma based on structural position, matching Jackson's actual `writeStringField` contract. Callers that followed the legacy convention (only for subsequent fields) produce identical output; callers that previously emitted invalid JSON by misusing the API now emit correct JSON. **Backward compatibility preserved** for all existing custom writers across the test suite (4083 + 165 + 28 = 4276 tests pass; the failure modes that surfaced during development — custom writers mixing context.writeImpl with Jackson API — are all fixed by the snapshot/restore wrapper). **Net code reduction**: -98 LOC in JsonWriter (parallel state machine + writeKey gone). Same-session adjacent A/B JsonPerformanceTest: Writes within ±4%, Reads within ±2% — perf flat within noise. The snapshot/restore wrapper adds ~10 ns of per-writeImpl overhead (allocation-free thanks to the arena), amortized across the per-document recursion depth.
* **REFACTOR**: Follow-up cleanup to step 2 — `Writers.java` (3 sites) and `WriteOptionsBuilder.java` (1 site) now call `CharStreamGenerator.writeJsonUtf8String` / `writeBasicString` directly instead of routing through `JsonWriter`'s `@Deprecated` public static delegates. JsonWriter retains the `@Deprecated` delegates for any external (library-consumer) caller. `JsonWriter.writeJson5String` is **deleted outright** — the `@Deprecated` method had zero callers internally after the `writeStringValue` migration; 4.103.0 is still unreleased so external breakage risk is minimal; external users needing smart-quote string emission should switch to `JsonGenerator.writeString(String)` via `JsonIo.createGenerator(...)` (it applies the same selection automatically). Net diff: -38 lines (the `writeJson5String` method body) + 4 small call-site updates. All 4083 tests pass; 0 failures; single perf run flat within run-to-run variance.
* **REFACTOR**: Step 2 of the JsonWriter dog-food migration — string-emission helpers moved from `JsonWriter` (public static) to `CharStreamGenerator` (package-private static). gen now owns the escape-scan, quote-style, and Unicode-handling logic for its own string emission rather than reaching across to JsonWriter as a utility namespace. The three escape lookup tables (`ESCAPE_STRINGS` / `CONTROL_UNICODE_ESCAPES` / `NEEDS_ESCAPE_SINGLE_QUOTE`), their static initializer, the `HEX_DIGITS` / `toUnicodeEscape` helpers, and the three active method implementations (`writeBasicString`, `writeJsonUtf8String` × 2 overloads, `writeSingleQuotedString`) live on `CharStreamGenerator` now. JsonWriter retains `@Deprecated` thin public static delegates that forward to gen — preserves backward compat for any external caller (and `Writers.java` / `WriteOptionsBuilder.java`'s legacy call sites). `writeJson5String` is marked `@Deprecated` (no internal callers after the writeStringValue migration; external users should switch to `JsonGenerator.writeString(String)` via `JsonIo.createGenerator` which applies the same smart-quote logic). The previously-dead `NEEDS_ESCAPE` boolean[] table is deleted (orphaned static field, no callers). 13 JsonWriter internal call sites updated from bare `writeJsonUtf8String(...)` / `writeBasicString(...)` to `CharStreamGenerator.writeJsonUtf8String(...)` / `writeBasicString(...)` to avoid self-deprecation warnings. gen's own 3 call sites (writeKey + writeString) drop the `JsonWriter.` qualifier (they're now calling local static methods). Same-session adjacent A/B JsonPerformanceTest: all six metrics within ±2% (perf-flat as expected for a pure code-relocation change). All 4083 tests pass; 0 failures.
* **REFACTOR**: Remove dead precomputed-prefix code in `JsonWriter` left over from the step-1 dog-food migration. After `writeId`, `writeType`, `writeOptionalReference`, and `writePrimitive`'s Long-wrap branch all moved to drive emission through `CharStreamGenerator`'s Jackson-style API (with gen making the key-quoting decision from a bare key name like `@id` / `$id` / `@i` / `$i`), the 3 instance fields (`idPrefix`, `typePrefix`, `refPrefix`) and their 18 backing static constants (`ID_SHORT`/`ID_LONG`/`ID_JSON5`/`ID_JSON5_SHORT`/`ID_DOLLAR_QUOTED`/`ID_DOLLAR_SHORT_QUOTED` × {id, type, ref}) had no callers. Removed. The 6-branch switch in the constructor shrinks by 3 assignments per branch (-18 lines of assignments). `ITEMS_*` / `KEYS_*` constants stay alive — `itemsPrefix` / `keysPrefix` are still consumed by the @items / @keys legacy emission paths that haven't migrated yet. Pure source-level cleanup; no runtime behavior change. All 4083 tests pass; 0 failures.
* **FEATURE**: `JsonWriter.writePrimitive`'s Long-wrap branch (the `Long && writeLongsAsStrings` path that wraps a long as `{"@type":"long","value":"..."}` when `showType` is on) migrated to drive the entire emission through `CharStreamGenerator`. Completes the dog-food migration of writePrimitive — the Long-wrap branch was deferred at D3 step 2 (commit `e10cff3f`) pending the `@type` prefix-alignment work that subsequently landed for writeId/writeType (`4dea9c62` / `04d1809d`). New path: `gen.resetForBridgeAtValueSlot()` + (if `showType`: `gen.writeStartObject() + gen.writeStringFieldUnescaped(typeKey, "long") + longBoxedWriter.write(obj, true, gen, this) + gen.writeEndObject()`) else (`longBoxedWriter.write(obj, false, gen, this)`). The longBoxedWriter call goes through the existing `JsonClassWriter`-on-`JsonGenerator` API that `Writers.PrimitiveTypeWriter.write(...)` provides (migrated in the Tier 3 chunk A1 commit) — the long-as-quoted-string emission happens through `gen.writeString(Long.toString(value))` inside `LongWriter.writePrimitiveForm`. State machine is engaged across the entire wrapper + value emission. With this commit, the last leaf in the step-1 set of the JsonWriter dog-food migration is complete (writeStringValue, writePrimitive incl. Long-wrap, writeId, writeType, writeOptionalReference @ref; writeLongDirect/writeIntDirect were already deleted in the digit-pair commit). Same-session JsonPerformanceTest single-run adjacent A/B: all six metrics flat within ±1% (writeLongsAsStrings is off by default so the migrated branch isn't exercised in the standard benchmark — perf change reflects only the unchanged code paths). All 4083 tests pass; 0 failures.
* **FEATURE**: `JsonWriter.writeOptionalReference`'s `@ref` branch migrated to drive the entire ref-object emission through `CharStreamGenerator` — third leaf in the dog-food migration after writeId / writeType. New path is `gen.resetForBridgeAtValueSlot(); gen.writeStartObject(); gen.writeNumberField(refKey, id); gen.writeEndObject();` — self-contained value-slot object emission, full state machine engaged for the entire `{"@ref":<id>}` structure (not just the value side as in the prior `gen.writeLongRaw` path). JsonWriter precomputes a new `refKey` field (4 variants — `@ref` / `@r` / `$ref` / `$r`) alongside the existing `idKey` and `typeKey`; the 6-variant `refPrefix` stays for now (no other caller). The `final Writer output` local variable in the method is also removed (gen owns its own `out` reference). Pretty-print output for `@ref` objects may now span multiple lines (gen.writeEndObject emits newline+indent in pretty mode for non-empty objects); the existing `PrettyPrintTest` uses `isEqualToIgnoringWhitespace`, so fixture tests remain green. Same-session JsonPerformanceTest single-run adjacent A/B vs the writeType baseline at HEAD = 04d1809d: Write `c=true` 1.75x→1.81x (+3.4%), Write `c=false` 1.61x→1.66x (+3.1%), Maps Write `c=true` 1.89x→1.97x (+4.2%), Maps Write `c=false` 1.60x→1.67x (+4.4%), Full Java Read flat, Maps Read 1.40x→1.31x (-6.4%, huge swing confirming the run was noisier than typical). Critically: `cycleSupport=false` short-circuits writeOptionalReference before reaching the migrated `@ref` branch — meaning the +3-4% delta on `c=false` Writes is pure noise (the migration physically cannot affect that path). With c=false setting the noise floor at 3-4%, the c=true Write deltas of similar magnitude are within noise. Net: **perf flat within noise**. All 4083 tests pass; 0 failures.
* **FEATURE**: `JsonWriter.writeType` migrated to a new package-private `gen.writeStringFieldUnescaped(name, value)` fast path — second leaf in the dog-food migration to drive a JsonWriter emission method through `CharStreamGenerator`'s state-machine-aware API while still preserving perf. The helper mirrors the public `writeStringField` but skips the per-call escape scan that `writeJsonUtf8String` would otherwise perform on the value. Trade-off: caller MUST guarantee the value is JSON-safe (no embedded `"`, `\`, or control chars below 0x20); Java type aliases / class names always satisfy this, so the trade is clean for the writeType use case. State machine is still fully engaged via `writeFieldName` for the key emission — same Jackson invariant as `writeId`. Pattern mirrors the `writeXxxRaw` family for scalars: the public Jackson API (`writeStringField`) stays safe and comprehensive for external consumers; an intra-package fast path lets callers that can prove safety skip the unnecessary work. The new path: `gen.resetForBridgeInsideObjectBody(this.depth); gen.writeStringFieldUnescaped(typeKey, alias);`. JsonWriter precomputes a new `typeKey` field (4 variants — `@type` / `@t` / `$type` / `$t`) alongside the existing `idKey`; the 6-variant `typePrefix` stays for now. The `Writer output` parameter on `writeType` is dropped (11 internal callers updated). A first attempt at this migration used the public `gen.writeStringField` and saw a +4-7% Write regression due to the escape scan; the fast-path approach (this commit) recovers the perf. Same-session JsonPerformanceTest single-run adjacent A/B vs the writeId-baseline at HEAD = 4dea9c62: Write `c=true` 1.75x→1.76x (+0.6%, flat), Write `c=false` 1.67x→1.64x (-1.8%, faster), Maps Write `c=true` 1.93x→1.85x (**-4.1% faster**), Maps Write `c=false` 1.66x→1.66x (flat); Reads moved ±0.5-0.8% (noise floor; Reads untouched). 2 new tests in `JsonGeneratorTest` cover the helper's emission (`writeStringFieldUnescaped_emitsRawValueSkippingEscapeScan` + `_nullEmitsJsonNull`). All 4083 tests pass; 0 failures.
* **FEATURE**: `JsonWriter.writeId` migrated to the Jackson-aligned `gen.writeNumberField(idKey, id)` — first leaf in the dog-food migration to fully drive a JsonWriter emission method through `CharStreamGenerator`'s **public** streaming-write API (vs the prior raw-helper bypasses used by writeStringValue / writePrimitive). The new path is `gen.resetForBridgeInsideObjectBody(this.depth); gen.writeNumberField(idKey, id);` — gen does the key-emission quoting decision (always double-quoted for `@id`/`@i` since they're not valid JSON5 identifiers, unquoted for `$id`/`$i` under `json5UnquotedKeys`), engages its structural state machine, and emits the digit-pair value. JsonWriter precomputes a new `idKey` field (4 variants — `@id` / `@i` / `$id` / `$i`) alongside the existing 6-variant `idPrefix`; the prefix constants stay for now (writeType/writeOptionalReference still use them pending similar migration). The previous `out.write(idPrefix) + gen.writeLongRaw(id)` path emitted the precomputed prefix string directly — fast but bypassed gen's structural state machine, leaving gen's `contextStack` permanently out of sync with JsonWriter's actual emission position. After this commit, gen's state machine is correctly synchronized via `resetForBridgeInsideObjectBody` for every `@id` emission. Same-session JsonPerformanceTest single-run adjacent A/B (HEAD = 337d15fa baseline vs writeId-migrated): Write `c=true` 1.86x→1.72x (-7.5% **faster**), Write `c=false` 1.68x→1.58x (-6.0%), Maps Write `c=true` 1.96x→1.85x (-5.6%), Maps Write `c=false` 1.67x→1.56x (-6.6%); Reads moved -1% to -5% but Read paths are untouched so direction confirms run-to-run noise. The migration is **perf-positive**, not just structurally cleaner — likely because the entire `gen.writeNumberField` call chain is fully inlinable by the JIT (all `final`, monomorphic) and composes better than two-call legacy approach (`out.write(prefix)` + separate `gen.writeLongRaw`). All 4081 tests pass; 0 failures.
* **CORRECTNESS**: `CharStreamGenerator.writeFieldName` / `writeKey` now always emit keys as either an unquoted JSON5 identifier (when both eligible AND in `json5UnquotedKeys` mode) or a double-quoted JSON UTF-8 string — never single-quoted, even when `json5SmartQuotes` is on. Previously the writeKey path had a third branch that single-quoted keys whenever `json5SmartQuotes` was enabled; this diverged from `JsonWriter.writeKey` (which has always double-quoted) and from Jackson's convention (keys are always double-quoted). The divergence broke the dog-food migration of `writeId`/`writeType` onto `gen.writeNumberField`/`writeStringField`: with `json5()` enabling both flags, "@id" (not a valid JSON5 identifier — starts with `@`) was emitted as `'@id':` through gen but `"@id":` through JsonWriter for the same input. Aligning gen to always double-quote keys removes the divergence. Smart-quote logic continues to apply to string *values* as before. 1 new test in `JsonGeneratorTest` (`json5SmartQuotes_doesNotApplyToKeys`) pins the contract; existing `json5UnquotedKeys_validIdentifier` / `json5UnquotedKeys_invalidIdentifier_stillQuoted` cases unchanged.
* **FEATURE**: `JsonWriter.writePrimitive` non-Long-wrap branches migrated to `CharStreamGenerator`'s new `writeXxxRaw` helper family — second leaf in the JsonWriter dog-food migration onto the `JsonGenerator` API (D3 step 2). New package-private state-machine-free emit helpers added on `CharStreamGenerator`: `writeBooleanRaw(boolean)`, `writeNullRaw()`, `writeDoubleRaw(double)`, `writeFloatRaw(float)` — counterparts to the existing `writeIntRaw` / `writeLongRaw`. Each public scalar method (`writeBoolean`, `writeNull`, `writeNumber(double)`, `writeNumber(float)`) now delegates to its raw helper after the state-machine bookkeeping, matching the int/long pattern. `writePrimitive` calls the raw helpers directly — bypasses `startValueContext + markValue` for these isolated single-value emissions JsonWriter handles its own structural context around. Two paths retained on legacy `out.write`: (a) the Long-wrap branch (`Long && writeLongsAsStrings && showType`) emits `{"@type":"long","value":"..."}` and is blocked on the `@type` prefix alignment that defers `writeId`/`writeType` migration; (b) the SMALL_INT_STRINGS cache fast path for tiny Integer values (also used by `writeField` + `writePrimitiveFieldDirect`). Per-call cost is strictly ≤ legacy (Integer/Long allocations eliminated, other types same), strictly < a state-machine-aware migration would have been. Same-session JsonPerformanceTest median-of-3 A/B vs the gen-alignment baseline: Write `c=true` -1.1% (1.76→1.74), Write `c=false` -1.3% (1.60→1.58), Maps Write `c=true` -3.1% (1.91→1.85), Maps Write `c=false` flat (1.60→1.60); Reads moved with similar magnitude but in inconsistent directions, confirming the deltas are within noise. All 4080 tests pass; 0 failures.
* **CORRECTNESS**: `CharStreamGenerator.writeNumber(double)` and `writeNumber(float)` now permit emitting `NaN` / `Infinity` / `-Infinity` literals when *either* `allowNanAndInfinity` is true *or* `json5InfinityNaN` is true. Previously only `isAllowNanAndInfinity` was captured at the generator level; `JsonWriter.isNanInfinityAllowed()` has always treated both flags as equivalent for emission ("|| writeOptions.isJson5InfinityNaN()"), but the generator's captured flag did not. The mismatch broke any consumer that set only `json5InfinityNaN(true)` and then drove emission through gen: gen rejected with `JsonGenerationException("Cannot serialize non-finite double (NaN/Infinity) when allowNanAndInfinity=false")` where JsonWriter would have accepted. Aligning the two paths is the foundational invariant for dog-fooding `JsonWriter.writePrimitive` onto gen (next leaf in the migration). 3 new tests in `JsonGeneratorTest` cover the `json5InfinityNaN`-alone branch (double + float) and the both-flags-set branch.
* **PERFORMANCE**: Digit-pair integer-to-chars conversion moved from `JsonWriter.writeLongDirect` / `writeIntDirect` into `CharStreamGenerator.writeLongRaw(long)` / `writeIntRaw(int)`. Three benefits: (a) `CharStreamGenerator.writeNumber(int)` and `writeNumber(long)` — the public streaming-write number-emission methods — now use the digit-pair algorithm directly, eliminating the per-call `Integer.toString` / `Long.toString` allocation that was the previous implementation; benefits ALL external `JsonGenerator` consumers, not just JsonWriter's internals. (b) JsonWriter consolidates onto a single source of truth for digit emission — the `DIGIT_TENS` / `DIGIT_ONES` lookup tables, the per-instance `longBuffer` char[] scratch, and the negative-space `Long.MIN_VALUE`-safe algorithm now live exclusively on `CharStreamGenerator`. Net code reduction in `JsonWriter` (-117 lines: 2 method bodies, 2 lookup tables, 1 scratch field, 2 Javadocs). (c) Unblocks subsequent dog-food migrations of `writeId`, `writeType`, `writeOptionalReference`, `writePrimitive`, `writeField` numeric paths — they can now call `gen.writeLongRaw` / `gen.writeIntRaw` at non-value-slot positions (after a precomputed prefix, mid-object-body, array element) without re-introducing the allocation overhead the digit-pair logic was added to avoid. 11 new tests in `JsonGeneratorTest` cover the algorithm edge cases: zero, single-digit positive/negative, the 100-boundary that enters the digit-pair loop, `Long.MIN_VALUE` / `Integer.MIN_VALUE` (regression-anchors for the negative-space arithmetic), `Long.MAX_VALUE` / `Integer.MAX_VALUE`, and several around the 4-digit boundary. Empirical perf flat on `mvn test` cumulative stats: median-of-5 same-session A/B Write 94.37 → 97.59 ms (+3%, post-migration range 89.94-107.45 straddles baseline range 91.57-100.93 — classic JIT-timing variance, not a systematic shift), Read 257.62 → 255.55 ms (-0.8%, expected to be flat since Read paths don't touch this change). Theoretical analysis confirms no measurable change: gen is a `final` field of a `final` class, the call is monomorphic and JIT-inlinable, the only logical difference per call is one additional field load (`this.gen` then `gen.longBuffer` vs the previous direct `this.longBuffer`) which the JIT CSE's to a no-op.
* **FEATURE**: `JsonWriter.writeStringValue` migrated to delegate string emission to `CharStreamGenerator` via the bridge-reset hook — first leaf in the JsonWriter dog-food migration onto the `JsonGenerator` API (D3 step 1). New body is `gen.resetForBridgeAtValueSlot(); gen.writeString(s);` — generator emits the quoted string content (with smart quote-style selection in `json5SmartQuotes` mode), JsonWriter remains responsible for surrounding separator + indent emission. The `resetForBridgeAtValueSlot` hook (introduced in D2 for `writeCustom` dispatch) keeps the generator's structural-state machine synchronized with JsonWriter's actual emission position so subsequent gen calls don't reject. All 12 internal call sites of `writeStringValue` (writePrimitiveArray, writeJsonObjectArray, writeMap variants, writeCollectionElement, writeEnumSet, writeField, writePrimitiveFieldDirect, writeStringField, writeValue) keep their original surrounding logic — only the inner content-emission path moved. No observable output change: A/B median-of-3 `mvn test` perf flat within run-to-run variance envelope (~10%); all 4259 tests pass across json-io + spring + spring-ai modules. Same-codebase invariant test (`json5SmartQuotes_matchesJsonWriterWriteStringValue` in `JsonGeneratorTest`) continues to pass — `gen.writeString` and `JsonWriter.writeStringValue` now share the emission path by construction.
* **CORRECTNESS**: `CharStreamGenerator.writeString(String)` now applies the same smart quote-style selection in `json5SmartQuotes` mode that `JsonWriter.writeStringValue` has always applied — picks single quotes only when the string contains `"` but not `'` (the quote style that minimizes escaping), and defaults to double quotes otherwise. Previously the generator unconditionally emitted single-quoted strings whenever `json5SmartQuotes=true`, which contradicted the flag name (*Smart*Quotes) and produced output divergent from `JsonIo.toJson(...)`'s string emission for the same input. Aligning the two paths is the foundational invariant for dog-fooding `JsonWriter` onto `CharStreamGenerator` — `JsonWriter.writeStringValue` can now delegate to `gen.writeString(value)` without changing observable output. `JsonWriter.shouldUseSingleQuotedString` was promoted from `private static` to package-private `static` so the generator can call it directly without code duplication. 6 new tests in `JsonGeneratorTest` cover the four smart-selection branches (no quotes / contains `"` only / contains `'` only / contains both), the empty-string corner case, and an `assertEquals(viaJsonIo, viaGen)` cross-path invariant test that pins the contract.
* **FEATURE**: `JsonIo.formatJson` reimplemented as a parse-and-re-emit pipeline through `JsonTokenizer` + `JsonGenerator` — the new public-API completion of the dog-food migration on the formatting path. The previous `JsonPrettyPrinter` character-shuffling class (214 LOC, deleted from `json-io/src/main/java/com/cedarsoftware/io/prettyprint/`) walked the input character-by-character and inserted whitespace around structural punctuation; the new implementation parses tokens via `JsonIo.createTokenizer(...)`, re-emits each via `JsonGenerator.copyCurrentEvent(t)` to a `JsonGenerator` with `prettyPrint=true`, and returns the resulting string. Single source of truth for json-io's pretty-print format. New `JsonIo.formatJson(String, WriteOptions)` overload lets callers supply custom `WriteOptions` (e.g. `indentationSize`, JSON5 flags); the single-arg `formatJson(String)` uses defaults. `JsonIoException` is thrown on parse failure (previously the character-shuffler returned malformed output silently for malformed input — the new path is stricter, which is arguably a correctness improvement).
* **FEATURE**: `Writers.PrimitiveTypeWriter.write(...)` migrated to the `JsonGenerator` API — the last piece of the writers/`Writers.java` migration story. `write()` now dispatches through `gen.writeFieldName(getKey())` + `writePrimitiveForm(obj, gen, context)`; the deprecated `Writer`-based override below remains as a bridge-generator delegate, with the showType-aware bridge-factory choice — `bridgeInsideObjectBody` when `showType=true` (called from inside an open `{...}` envelope), `bridgeAtValueSlot` when `showType=false` (called from a value slot, e.g. `JsonWriter.writePrimitive`'s `longBoxedWriter` case or the `doubleWriter` / `floatWriter` / `longPrimitiveWriter` element-write loops in `writeObjectArray`). Unblocked by the `JsonIo.formatJson` and `CharStreamGenerator` pretty-print alignment work shipped in the same release.
* **FEATURE**: `CharStreamGenerator` pretty-print now follows the standard Jackson convention — scalar values stay on the same line as their field name (`"name": "value"`), only object/array opening tokens and array elements get newline+indent. The previous behavior put every value on a new line (`"name": \n  "value"`) which is more verbose than Jackson's default and didn't match json-io's own historical compact-after-field-name format. Aligns `JsonIo.toJson(..., prettyPrint=true)` (streaming/tree paths) with the new `JsonIo.formatJson(...)` (tokenizer/generator path) so both produce the same standard pretty-print format. `prettyPrint.json` test fixture updated to reflect the new format (space-after-colon throughout); `PrettyPrintTest`'s `formatJson == prettyPrintDirect` assertion relaxed from `isEqualToIgnoringNewLines` to `isEqualToIgnoringWhitespace` to account for the few `JsonWriter` tree-walker code paths that haven't migrated to drive a `JsonGenerator` yet (those still emit the legacy compact-after-field-name format — same JSON content, slightly different inter-token whitespace).
* **FEATURE**: `JsonGenerator.deprecatedWriterBridge_insideObjectBody` and `deprecatedWriterBridge_atValueSlot` now tolerate a `null` `WriteOptions` argument by falling back to `WriteOptionsBuilder.getDefaultWriteOptions()` — preserves the contract of the legacy `Writer`-based custom-writer API which tolerates a `null` `WriterContext`. Previously, calling a migrated writer's deprecated method with a null context threw `NullPointerException` from `CharStreamGenerator`'s constructor (e.g. `WritersTest.testUnusedAPIs`). (This entry was first added with chunk A1; carrying forward in the consolidated 4.103.0 narrative.)
* **REFACTOR**: Deleted `com.cedarsoftware.io.prettyprint.JsonPrettyPrinter` (214 LOC). All callers — `JsonIo.formatJson` and `FormattingLimitsTest`'s formatting-options tests — now route through the `JsonTokenizer`+`JsonGenerator` path. The `prettyprint` package directory is removed.
* **FEATURE**: `Writers.java` misc-types migration (Tier 3 chunk A4) — final 6 `PrimitiveTypeWriter` subclasses migrated: `Writers.LocaleWriter`, `Writers.BigIntegerWriter`, `Writers.PatternWriter`, `Writers.CurrencyWriter`, `Writers.BigDecimalWriter`, `Writers.UUIDWriter`. Each writer now exposes the `JsonGenerator`-based `writePrimitiveForm(...)` with the real emission logic; the `Writer`-based form is retained as a `@Deprecated` bridge-generator delegate. `BigInteger` and `BigDecimal` continue to emit as quoted JSON strings (not number literals) so JS / Jackson clients that store numbers as doubles do not lose precision on large arbitrary-scale values — explicitly documented in the migrated source. With this chunk, every `JsonClassWriter` implementation in `writers/` and the leaf subclasses of `PrimitiveTypeWriter` in `Writers.java` are on the new dispatch path. The base class `Writers.PrimitiveTypeWriter.write()` itself is the one remaining piece on the legacy path; an exploratory migration of it during this chunk surfaced a dispatch nuance (the framework calls `write()` from contexts the bridge generator's stricter structural-state machine doesn't accept), so that migration is deferred to a focused investigation in a later commit. Until then `PrimitiveTypeWriter.write()` retains its hand-rolled `output.write(...)` punctuation and remains correct.
* **FEATURE**: `Writers.java` java.time / Date-related writers migration (Tier 3 chunk A3) — 15 `PrimitiveTypeWriter` subclasses that define their own `writePrimitiveForm` now expose the `JsonGenerator`-based form with the real emission logic; the `Writer`-based form is retained as a `@Deprecated` bridge-generator delegate. Classes migrated: `LocalDateAsLong`, `LocalDateWriter`, `LocalTimeWriter`, `LocalDateTimeWriter`, `ZonedDateTimeWriter`, `YearMonthWriter`, `MonthDayWriter`, `OffsetTimeWriter`, `OffsetDateTimeWriter`, `InstantWriter`, `ZoneOffsetWriter`, `DurationWriter`, `PeriodWriter`, `YearWriter`, `TimestampWriter`. Direct replacements: `JsonWriter.writeJsonUtf8String(output, str)` → `gen.writeString(str)`; `output.write(Long.toString(value))` → `gen.writeNumber((long))` (eliminates the intermediate `Long.toString` allocation in `LocalDateAsLong`'s epoch-milli emission). Null-context tolerant on every bridge. Behavior unchanged across the existing `WritersTest` coverage for these types (`testLocalDateTimeToJsonObject`/`String`, `testLocalDateToJsonObject`/`String`, `testLocalTimeToJsonObject`/`String`, `testOffsetTimeToJsonObject`/`String`, `testZonedDateTimeToJsonObject`/`String`, `testOffsetDateTimeToJsonObject`/`String`, `testInstantToJsonObject`/`String`, `testMonthDayToJsonObject`/`String`, `testYearMonthToJsonObject`/`String`, `testPeriodToJsonObject`/`String`, `testYearToJsonObject`/`String`, `testZoneOffsetToJsonObject`/`String`, `testDurationToJsonObject`/`String`).
* **FEATURE**: `Writers.java` Date/Calendar trio migration (Tier 3 chunk A2) — `Writers.CalendarWriter`, `Writers.DateWriter`, and `Writers.DateAsLongWriter` (each implements `JsonClassWriter` directly, not via `PrimitiveTypeWriter`) now expose both an object-body `write(...)` and a primitive-form `writePrimitiveForm(...)` on the `JsonGenerator` API with the real emission logic; the deprecated `Writer`-taking overrides are retained as bridge-generator delegates. Behavior unchanged — covered by `WritersTest`'s `testCalendarToJsonObject`/`testCalendarToJsonString`/`testDateToJsonObject`/`testDateToJsonString`/`testSqlDateToJsonObject`/`testSqlDateToJsonString` plus `testTimestampToJsonObject`/`testTimestampToJsonString` (`Timestamp` extends `Date`). The `DateAsLongWriter` millisecond emission switches from a hand-rolled `output.write(Long.toString(...))` to `gen.writeNumber((long))` — same JSON output, no allocation for the intermediate `String`. All three classes are null-context tolerant in the bridge (consistent with chunk A1's bridge-factory bug fix).
* **FEATURE**: `Writers.java` base-class migration (Tier 3 chunk A1) — `Writers.PrimitiveValueWriter`, `Writers.FloatingPointWriter`, `Writers.PrimitiveUtf8StringWriter`, and `Writers.CharacterWriter` each gain a new `JsonGenerator`-based `writePrimitiveForm(...)` carrying the real emission logic; the existing `Writer`-based `writePrimitiveForm(...)` overrides are retained as `@Deprecated` bridge-generator delegates so user subclasses written against the old API continue to chain via `super.writePrimitiveForm(o, output, ctx)` unchanged. The migration puts 7 leaf writers on the new dispatch path "for free" via inheritance: `FloatWriter` and `DoubleWriter` (via `FloatingPointWriter`) and `TimeZoneWriter`, `ClassWriter`, `EnumsAsStringWriter`, `ZoneIdWriter`, `JsonStringWriter` (via `PrimitiveUtf8StringWriter`). `ZoneIdWriter` was the last unmigrated writer in the `writers/` package; with this chunk every writer in `writers/` now routes through the new API. Behavior unchanged across the full 4061-test main suite. Direct `PrimitiveTypeWriter` subclasses that have their own `writePrimitiveForm` (`LocalDateWriter`, `BigDecimalWriter`, `UUIDWriter`, `CalendarWriter`, `DateWriter`, etc.) still use the legacy path and migrate in subsequent chunks; `PrimitiveTypeWriter` itself (the base `write(...)`) is left untouched until all its `writePrimitiveForm`-overriding descendants have migrated, since `PrimitiveTypeWriter.write(JsonGenerator)` would need to invoke `writePrimitiveForm(JsonGenerator)` virtually on subclasses that haven't yet provided the new override.
* **BUG FIX**: `JsonGenerator.deprecatedWriterBridge_insideObjectBody(Writer, WriteOptions)` and `JsonGenerator.deprecatedWriterBridge_atValueSlot(Writer, WriteOptions)` now tolerate a `null` `WriteOptions` argument by falling back to `WriteOptionsBuilder.getDefaultWriteOptions()`. The legacy `Writer`-based custom-writer API (e.g. `JsonClassWriter.writePrimitiveForm(Object, Writer, WriterContext)`) tolerates a `null` `WriterContext` (and therefore a `null` options derivation); the bridge factories needed to preserve that contract for the deprecated-method delegate pattern to remain correct. Without this fix, `PrimitiveUtf8StringWriter.writePrimitiveForm(obj, writer, null)` — exercised by `WritersTest.testUnusedAPIs` — threw `NullPointerException` from `CharStreamGenerator`'s constructor reading `writeOptions.isPrettyPrint()`.
* **FEATURE**: `JsonGenerator.writeObject(Object)` — Jackson-aligned databind entry point on the streaming-write API. Drives the same graph-serialization code path as `JsonIo.toJson` (cycle tracking, `@id`/`@ref` identity preservation, custom-writer dispatch, `@type` policy), so an arbitrary POJO can be embedded inside a hand-written streaming sequence with one call rather than the previous `gen.writeRawValue(JsonIo.toJson(o))` workaround. Honors the active `ShowType` policy via the new `protected getWriteOptions()` accessor — same lever that `writeBinary` reads. A `null` input emits the JSON literal `null`. v1 implementation is `writeRawValue(JsonIo.toJson(o, getWriteOptions()))` — correct and complete but builds an intermediate `String`; the dog-food refactor (JsonWriter driving JsonGenerator directly, eliminating the intermediate) is deferred to a future release. `Class`- and `Type`-accepting overloads (the streaming-write mirror of `JsonIo.toJava(...).asClass(...)` / `.asType(...)`) are also deferred — they require deeper plumbing into JsonWriter's showType-decision logic. Identity tracking is currently per-call: two separate `writeObject` calls with the same instance serialize the instance twice rather than sharing an `@id`/`@ref`; that limitation will be addressed when the dog-food migration lands. The class-level Javadoc bullet that previously called out "No databind `writeObject(Object)` entry point" as a deliberate Jackson divergence has been rewritten to describe the new capability. 8 new tests in `JsonGeneratorTest` cover top-level POJO round-trip, null handling, use inside an object body, auto-comma across multiple `writeObject` calls inside an array (with array round-trip), `showTypeInfoNever()` honoring, and a side-by-side default-vs-Never regression-anchor.
* **FEATURE**: `JsonGenerator.writeObjectField(String, Object)` — convenience for `writeFieldName(name) + writeObject(value)`, mirroring the existing `writeStringField` / `writeNumberField` / `writeBooleanField` family. Especially useful when building objects field-by-field where some fields are POJOs and others are scalars.
* **FEATURE**: `JsonGenerator.writeBinary(byte[])` (and the slice overload) now honor the active `WriteOptions.isNeverShowingType()` policy. Under the default `showTypeInfoMinimal()` / `showTypeInfoMinimalPlus()` / `showTypeInfoAlways()` modes, `writeBinary` still emits the json-io-native wrapped form `{"@type":"byte[]","value":"<base64>"}` for unambiguous round-trip. Under the explicit Jackson-compatible mode `showTypeInfoNever()`, `writeBinary` instead emits a bare base64 string `"<base64>"` matching Jackson's wire format — the receiver recovers the bytes via java-util's smart `String → byte[]` detection at the Converter layer. Implemented by adding a `protected WriteOptions getWriteOptions()` accessor on the abstract `JsonGenerator` (null-tolerant default for degenerate/test subclasses); `CharStreamGenerator` overrides to return its retained `WriteOptions` reference. The `null` short-circuit in `writeBinary` happens before the policy check, so `writeBinary((byte[]) null)` continues to emit the JSON literal `null` in both modes. Six new tests in `ByteArrayRoundTripTest` cover the bare emission, round-trip via smart detection, default-mode regression-anchor, null behavior under `Never`, the slice overload's inheritance of the policy, and a head-to-head side-by-side test that pins exact JSON output for both default and `Never` modes from the same input — a regression-anchor that fails loudly if the policy plumbing is ever broken in a future refactor.
* **DOCUMENTATION**: Cleaned up 8 latent `mvn javadoc:javadoc` warnings — the json-io main module now produces zero Javadoc warnings on a clean build. Fixes: `JsonGenerator` Javadoc had a broken `@link com.cedarsoftware.util.convert.MapConversions` reference (the target class is package-private in java-util, so the cross-module link couldn't resolve) — switched to `{@code MapConversions}` text reference. `JsonIo`: three single-arg overloads (`toJson(Object)`, `fromToon(InputStream)`, `toJava(InputStream)`) had stale `@param` lines (bogus parameter names, duplicate `@param in`) copied from their two-arg siblings — pruned to match each method's actual signature. `JsonToken`: four enum docs (`START_OBJECT` / `END_OBJECT` / `START_ARRAY` / `END_ARRAY`) used `{@code '{'}` style where the literal brace inside the `@code` tag confused the Javadoc parser — switched to the parser-safe `<code>'{'</code>` HTML form (rendered output unchanged). `JsonTokenizer` class doc referenced `JsonIo#createTokenizer(java.io.Reader)`, which doesn't exist (only `String` / `InputStream` overloads ship) — removed the broken cross-reference.
* **FEATURE**: `CompactMapWriter`, `CompactSetWriter`, `MultiKeyMapWriter` migrated to the new `JsonGenerator`-based `JsonClassWriter` API — completes the migration of all 19 outer writers in the `writers/` package. The `"config"` field on each is now emitted via the generator (`gen.writeFieldName` + `gen.writeString`); the data structure (a nested object of arbitrary entries for `CompactMap`, an array of arbitrary elements for `CompactSet`, an array of `{"keys":...,"value":...}` sub-objects for `MultiKeyMap`) continues to use the `WriterContext` semantic API since each element requires tree-walker recursion via `context.writeImpl` / `context.writeValue` / `context.writeObjectField` to handle arbitrary nested types — `JsonWriter`'s independent contextStack handles all structural commas inside the data structure during recursion. `CompactMapWriter` additionally drops its hand-rolled `output.write("\"config\":\"" + configStr + "\",")` / `output.write("\"data\":{")` / loop-internal `output.write(',')` / `output.write("}")` punctuation for the declarative `gen.writeFieldName` / `context.writeObjectFieldStart` / `context.writeFieldName` / `context.writeEndObject` form. Each writer's deprecated `Writer`-taking `write(...)` is retained as a bridge-generator delegate for `super.write(...)` chaining from user subclasses. Behaviour unchanged across all 44 targeted `CompactMapSetTest`, `CompactMapCustomTypeTest`, and `MultiKeyMapTest` cases plus the full 4047-test suite.
* **FEATURE**: `CharBufferWriter` migrated to the new `JsonGenerator`-based `JsonClassWriter` API. Replaces the `context.writeFieldName("value") + context.writeValue(str)` pair with the equivalent `gen.writeFieldName + gen.writeString`. CharBuffer position/limit preservation logic for array-backed vs direct buffers is unchanged.
* **FEATURE**: `CountDownLatchWriter`, `SemaphoreWriter`, `ReentrantLockWriter`, `ReentrantReadWriteLockWriter` migrated to the new `JsonGenerator`-based `JsonClassWriter` API. All four writers expose both an object-body `write(...)` and a primitive-form `writePrimitiveForm(...)` and now override both in `JsonGenerator` form; the deprecated `Writer`-taking overrides are retained as bridge-generator delegates. `SemaphoreWriter`'s `permits`+`fair` two-field emission collapses from six `output.write(...)` calls (with hand-managed comma) to four declarative generator calls (`writeFieldName`/`writeNumber`/`writeFieldName`/`writeBoolean`) with the generator handling the comma.
* **FEATURE**: `LongWriter`, `BitSetWriter`, `AtomicIntegerArrayWriter`, `AtomicLongArrayWriter` migrated to the new `JsonGenerator`-based `JsonClassWriter` API. All four are `Writers.PrimitiveTypeWriter` subclasses that emit a primitive form only; each writer now overrides BOTH `writePrimitiveForm` forms — new method has the real logic, old method delegates to it via the bridge generator. Code reduction is substantial: e.g. `LongWriter` drops the `output.write('"')`/`output.write(value)`/`output.write('"')` quoting dance for `gen.writeString(Long.toString(value))`; `AtomicIntegerArrayWriter`/`AtomicLongArrayWriter` drop the manual `[` / `,` / `]` punctuation for `gen.writeStartArray()` + `gen.writeNumber(...)` + `gen.writeEndArray()`. Behaviour is unchanged for all existing tests.
* **FEATURE**: `Optional`, `OptionalInt`, `OptionalLong`, `OptionalDouble` writers migrated to the new `JsonGenerator`-based `JsonClassWriter` API. Each writer's `write(...)` (legacy object form `{"present":bool,"value":x}`) and `writePrimitiveForm(...)` (Jackson/Gson-compatible bare value form) gain `JsonGenerator`-taking overrides with the real emission logic; the deprecated `Writer`-taking overrides are retained as thin delegates via the bridge generator for `super.write*()` chaining from user subclasses. Behaviour identical for all existing tests (23 Optional-family tests pass unchanged), with one latent-bug correction: `OptionalDouble.of(NaN)` / `Double.POSITIVE_INFINITY` etc. now honour the active `WriteOptions.allowNanAndInfinity` policy via the generator (previously the writer emitted bare `NaN`/`Infinity` tokens regardless of policy, which produced invalid JSON in strict mode).
* **FEATURE**: First two built-in custom writers migrated to the new `JsonGenerator`-based `JsonClassWriter` API: `ByteBufferWriter` (object-body form, emits the `"value":"<base64>"` field) and `ByteArrayWriter` (primitive form, emits the raw Base64 string). Each migrated writer keeps BOTH method overrides — the new `JsonGenerator`-taking form holds the real emission logic and the deprecated `Writer`-taking form delegates to it via a bridge generator — so user subclasses that override the deprecated method and chain via `super.write*(...)` continue to receive the built-in's behaviour unchanged. New public factory methods on `JsonGenerator`, `deprecatedWriterBridge_insideObjectBody(Writer, WriteOptions)` and `deprecatedWriterBridge_atValueSlot(Writer, WriteOptions)`, expose the bridge construction needed by the delegate pattern; Javadoc clearly flags these as migration scaffolding rather than primary streaming-write API (application code should use `JsonIo.createGenerator(...)` instead). `ByteArrayWriterTest` updated to exercise the new method directly via a real generator (the prior version passed a null `WriterContext`, which the legacy implementation ignored but the migrated delegate path requires for `getWriteOptions()`); other tests (`ByteBufferWriterTest`, `ByteArrayRoundTripTest`) cover the framework-dispatched path unchanged.
* **FEATURE**: `Writers.writeWithStringFormat`, `Writers.writeNumericWithFieldFormat`, and `Writers.writeWithFieldFormat` each gain a `JsonGenerator`-taking overload alongside the existing `Writer`-taking form. The new overloads emit through `gen.writeString(...)` so the generator's escape, quoting, and structural-state logic stays the single source of truth for well-formed JSON. The deprecated `Writer`-taking overloads continue to work and are marked `@Deprecated` for removal in 5.0. Built-in `Writers` templates (`PrimitiveTypeWriter`, `PrimitiveValueWriter`, `FloatingPointWriter`) still call the old forms — they migrate alongside the concrete writers that extend them in upcoming commits. 7 new tests in `WritersJsonGeneratorOverloadTest` cover the three new overloads against `String.format`, `DecimalFormat`, and `DateTimeFormatter` patterns plus the null-pattern / wrong-type fall-through paths.
* **FEATURE**: Framework dispatch is now wired up for the `JsonGenerator`-based `JsonClassWriter` overloads added in the previous commit. A new package-private `CustomWriterDispatch` class inspects each registered custom writer's runtime class once (walking the hierarchy with `getDeclaredMethod`), caches a per-class decision in a java-util `ClassValueMap`, and routes each `write` / `writePrimitiveForm` call to whichever overload the writer overrode. Rule: if a class declares the new (`JsonGenerator`-taking) form anywhere in its hierarchy, dispatch goes to that form; otherwise the deprecated `Writer`-based form. Writers that override BOTH (the pattern migrated built-in writers will follow to preserve `super.write*()` chaining from user subclasses) get the new form. `CharStreamGenerator` gains two package-private factory methods — `bridgeInsideObjectBody(Writer, WriteOptions, int currentDepth)` and `bridgeAtValueSlot(Writer, WriteOptions)` — that build a generator pre-seeded with the structural-stack state matching where `JsonWriter` has paused, including a one-shot `suppressNextIndent` flag so the bridge does not double-emit the leading newline+indent for its first field. `JsonWriter.writeCustom` now consults the dispatch info at both call sites (the `write` and `writePrimitiveForm` paths). Built-in writers are NOT migrated in this commit (deferred to subsequent 4.10x commits, one or a few writers at a time); the dispatch path is fully exercised end-to-end by new tests in `JsonGeneratorCustomWriterDispatchTest` (7 cases pairing a new-API writer against an old-API writer for the same model, including pretty-print). Old-API writers continue to take the exact same code path they did before — zero overhead beyond a single `ClassValueMap.computeIfAbsent` lookup on first dispatch per class.
* **FEATURE**: `JsonClassWriter` gains a Jackson-aligned, `JsonGenerator`-based emission API — the start of the dog-food migration that retires custom writers' direct dependence on `java.io.Writer`. Two new `default` methods: `write(T, boolean, JsonGenerator, WriterContext)` and `writePrimitiveForm(Object, JsonGenerator, WriterContext)`. The corresponding `Writer`-taking forms are marked `@Deprecated` and scheduled for removal in json-io 5.0; both remain fully functional during the 4.x migration period. Custom writers using the new API benefit from the generator's automatic JSON escape, structural-state machine (auto-commas), pretty-print, JSON5 unquoted-keys / single-quotes, NaN/Infinity policy, and `maxStringLength` enforcement — emitter code reduces to `gen.writeFieldName(name); gen.writeString(value);` style. This commit only adds the interface methods (default no-ops); the framework dispatch detecting which overload a writer has overridden lands in a follow-up commit, and built-in writers will migrate one at a time across subsequent 4.10x releases. Old 4.25.0-era code continues to compile and run unchanged (the deprecated `Writer`-based methods still exist on the interface; the legacy `JsonWriter.JsonClassWriter` nested-interface shim is preserved).
* **PERFORMANCE**: Cache-verify loops in `ToonReader.cacheSubstringFromBuf` and `CharStreamTokenizer.cacheStringFromChars` now route the post-bulk-extract `char[]`-vs-`char[]` equality through java-util 4.103.0's new `VectorizedArrays.equalsRange`, which dispatches at runtime to JDK 9+'s SIMD-vectorized `Arrays.equals(arr, int, int, arr, int, int)` intrinsic (JDK 8 falls back to a hand-rolled loop). Replaces our previous manual loop. Combined with the `CharBufScratch.getChars` bulk-extract already in place, the cache-verify step is now: one SIMD bulk copy of the cached String to a thread-local `char[]`, then one SIMD range-equals against the source buffer. Audit of json-io + java-util confirmed no additional range-equality hot spots: `String.regionMatches` callers are already coder-aware/optimal, `Arrays.equals` callers all operate on full arrays (already intrinsic), and the few residual `charAt` loops are either single-char checks or sequential forward parsers.
* **PERFORMANCE**: Cache-verify loops in `ToonReader.cacheSubstringFromBuf` and `CharStreamTokenizer.cacheStringFromChars` now bulk-extract the cached String's chars via java-util's `CharBufScratch.getChars(...)` (which uses `String.getChars` — a HotSpot intrinsic with SIMD on supported HW for compact-string `byte[]` → `char[]` expansion in one pass), then compare `char[]` to `char[]` in a tight JIT-vectorizable loop. Replaces per-char `cached.charAt(j)` (the LATIN1/UTF16 coder branch + per-char bounds check + virtual dispatch). Measured impact on the ToonReader method: dropped from 2.06% to 1.33% of CPU samples in JFR (-32.7% relative, fell from rank #4 to rank #9 in the hot-method list); `StringLatin1.charAt` overall dropped from 1.22% to 0.98% as a consequence. The two methods are the parallel cache-verify hot spots for the TOON reader and JSON tokenizer respectively; same fix applies symmetrically. Cache-hit path becomes substantially faster; cache-miss path is unchanged.
* **FEATURE**: `JsonGenerator.writeBinary(byte[])` (plus the `writeBinary(byte[], int offset, int length)` slice overload) — Jackson-aligned streaming-write method for byte arrays. Emits the wrapped form `{"@type":"byte[]","value":"<base64>"}` which round-trips cleanly through json-io's reader into the original `byte[]`. Default implementations on the abstract `JsonGenerator` class compose existing abstract methods (`writeStartObject` + `writeStringField` + `writeEndObject`) so no concrete `CharStreamGenerator` override is needed; auto-comma applies via the structural context machine, null input emits the JSON null literal. Counterpart to java-util 4.103.0's new `MapConversions.toByteArray` (the read-side wrapped-form decoder) and the smart String → byte[] detection.
* **FEATURE**: `ObjectResolver.assignField` now dispatches through Converter when the JSON value is a `JsonObject` carrying a `value` or `_v` key AND Converter knows how to build the target type (the wrapped-form contract used by `JsonGenerator.writeBinary` and the existing `ByteBuffer` tree writer). Routed BEFORE the recursive object-resolution path so primitive-array wrapped forms like `{"@type":"byte[]","value":"<base64>"}` reach `MapConversions` in java-util instead of falling into the generic class-factory path which left `byte[]` fields null. When the JsonObject has `@id`, the converted result is registered on the JsonObject via `setTarget(...)` so subsequent `@ref` resolutions find the same instance — preserves identity for shared-reference graphs across the wrapped form. The 'value'/'_v' key check distinguishes wrapped forms from the existing tree-writer `@items` form for shared byte[] (`{"@id":1,"@items":[1,2,3,4]}`), which continues to flow through the existing `assignJsonObjectField` path.
* **TESTING**: New `ByteArrayRoundTripTest` with 13 cases covering `JsonGenerator.writeBinary` round-trip (basic, null, 3-byte small blob to exercise the short-Base64 dispatch fallback, 1KB large blob, slice overload, OutputStream UTF-8 encoding), tree-writer number-array form continuity, foreign-input formats decoded via java-util's smart `String → byte[]` (Jackson-style bare Base64, hex magic numbers, spaced hex, short text fallback to charset), `ByteBuffer` field round-trip, and shared-reference preservation across the wrapped form via `@id`/`@ref`.
* **TESTING**: `java-json-benchmark` fork (branch `json-io-4.102.0-databind-coverage`) now exercises the streaming-write API through the standard JMH harness. `Serialization.jsonio()` switched from `JsonIo.toJson(...)` (tree) to `JsonIo.createGenerator(...)` driving a hand-rolled `UsersStreamSerializer.jsonio(JsonGenerator, Users)` method (~115 LOC, method-for-method port of the existing `jackson(...)` since both surfaces are Jackson-aligned). Round-trip validated end-to-end: Jackson's parser reads the streaming-write output back into the original `Users` POJO with deep-equals. First apples-to-apples streaming-write benchmark for json-io alongside the existing streaming-read entry. 1KB JMH spot-check vs comparable libs: jsonio 3.92 M ops/s, jackson 5.83 M ops/s (1.49× Jackson), gson 1.23 M ops/s (jsonio ~3.2× faster than gson). Benchmark-fork commit held back pending 4.103.0 Maven Central release (it depends on the new `createGenerator` API).
* **FEATURE**: Streaming-write API — `JsonGenerator` (Jackson-aligned cursor-style serializer). New `JsonIo.createGenerator(Writer)` / `createGenerator(Writer, WriteOptions)` / `createGenerator(OutputStream)` / `createGenerator(OutputStream, WriteOptions)` factories return a `JsonGenerator` instance for hand-rolled streaming output. Method set mirrors Jackson's `com.fasterxml.jackson.core.JsonGenerator`: `writeStartObject` / `writeEndObject` / `writeStartArray` / `writeEndArray` / `writeFieldName`, scalar `writeString` / `writeNumber` (int/long/float/double/BigInteger/BigDecimal/String) / `writeBoolean` / `writeNull`, raw `writeRaw` (3 overloads) / `writeRawValue`, convenience `writeStringField` / `writeNumberField` (4 overloads) / `writeBooleanField` / `writeNullField` / `writeArrayFieldStart` / `writeObjectFieldStart`, and a `copyCurrentEvent` / `copyCurrentStructure` bridge driven by a `JsonTokenizer` for parse → transform → emit pipelines. Fluent `JsonGenerator` return on every write method. Concrete implementation `CharStreamGenerator` is package-private. Deliberate Jackson divergences (documented in `JsonGenerator` Javadoc): no `writeObject(Object)` databind entry-point (use `JsonIo.toJson` for tree serialization, or `writeRawValue(JsonIo.toJson(value))` to splice tree output into a streaming sequence); no `writeBinary(byte[])` in v1 (emit base64 yourself; for round-trippable binary use `java.nio.ByteBuffer` which the tree writer handles automatically); `writeNumber(BigDecimal)` emits the canonical `stripTrailingZeros().toPlainString()` form unquoted (matches `Converter`'s canonical string form), `writeNumber(BigInteger)` emits `toString()` unquoted. Performance pattern matches `createTokenizer` (commit `e5700bad`): cached default `WriteOptions` (lazy-init volatile) + `BufferRecycler.borrowWriterCharBuffer` wrapped in a `FastWriter`, released back on close. 57 new tests in `JsonGeneratorTest` covering scalars, nesting, auto-comma, convenience helpers, raw vs raw-value, structural-misuse `JsonGenerationException` paths, pretty-printing, JSON5 unquoted keys / single quotes, NaN/Infinity policy, factory parity, close-flushes, UTF-8 round-trip, and bridge methods.
* **FEATURE**: Directional field-exclusion aliases on `ReadOptionsBuilder` and `WriteOptionsBuilder`. Both Jackson-named (Java-bean perspective) and unambiguous (JSON-direction) aliases now exist for the permanent and per-instance exclusion methods — pick whichever reads more clearly at the call site; all delegate to the same internal map. **Write side** (`WriteOptionsBuilder`): `addPermanentWriteOnlyField` / `addPermanentDeserializeOnlyField` alias `addPermanentExcludedField`; `addWriteOnlyField` / `addDeserializeOnlyField` alias `addExcludedField`. **Read side** (`ReadOptionsBuilder`): `addPermanentReadOnlyField` / `addPermanentSerializeOnlyField` alias `addPermanentNotImportedField`; `addReadOnlyField` / `addSerializeOnlyField` alias a new `addNotImportedField` (per-instance equivalent that didn't previously exist on the read side — filled the asymmetry). All twelve methods carry thorough Javadoc explaining the Java-bean perspective (Jackson's lens) vs. JSON-direction perspective, the equivalent annotations (`@IoProperty(access=...)` / `@IoIgnoreProperties(allowGetters=/allowSetters=)`), and a scope caveat: per-instance variants share the static field-injector/accessor cache, so `addPermanent*` at bootstrap is the path for authoritative global exclusion. Pre-existing aliasing bug fixed at the same time: `addExcludedField` and the new `addNotImportedField` use a `compute()`-with-fresh-Set pattern so they never mutate the BASE permanent-state Sets. 12 new tests in `DirectionalFieldExclusionTest`.
* **FEATURE**: Jackson-aligned directional ignore controls. `@IoProperty(access = Access.WRITE_ONLY)` accepts the field on input but suppresses it on output (mirrors `@JsonProperty(access = WRITE_ONLY)`); `Access.READ_ONLY` does the inverse. `@IoIgnoreProperties` gains `allowGetters` / `allowSetters` boolean escapes that mirror Jackson's same-named attributes — `allowGetters = true` keeps listed properties in output JSON while still blocking input; `allowSetters = true` does the opposite. Existing `@IoIgnore` (field) and zero-arg `@IoIgnoreProperties` (class) remain the both-sides shortcuts and are unchanged. The reflective metadata in `AnnotationResolver.ClassAnnotationMetadata` now exposes `isIgnoredOnRead(String)` / `isIgnoredOnWrite(String)` for callers that need directional precision; `isIgnored(String)` continues to return the both-sides union. Three call sites in `WriteOptionsBuilder`, `ReadOptionsBuilder`, and `Resolver` were retargeted to the directional methods so the new annotations are honored end-to-end. 20 new tests in `DirectionalIgnoreTest`.
* **PERFORMANCE**: `JsonIo.createTokenizer(...)` now caches the default `ReadOptions` (lazy-init, volatile) instead of building a fresh one via `new ReadOptionsBuilder().build()` per call when the caller passes null. Previously every streaming-parse invocation paid the full `ReadOptionsBuilder.build()` cost (loading aliases.txt, building ClassValueMaps, allocating the immutable snapshot), which dominated total work at small payloads. java-json-benchmark stream-deser of `Users` JSON, jsonio-vs-Jackson ratio: **1KB 34.5x → 1.96x**, **10KB 7.09x → 1.46x** (now faster than Gson), 100KB 1.94x → 1.71x, 1MB 1.72x → 1.76x (flat). All four sizes now sit at or better than Gson; the per-parse fixed cost is no longer visible to small-payload workloads.
* **PERFORMANCE**: `JsonIo.createTokenizer(...)` borrows the `FastReader` reader/pushback char buffers from the existing thread-local `BUFFER_RECYCLER` (same pattern the tree-builder paths use) instead of allocating fresh 65,536-char + 16-char buffers per call. Released back via the tokenizer's `close()` hook, so the typical try-with-resources usage gets the recycling automatically. Architectural consistency with the tree-builder; perf delta within JMH noise on its own at 1KB (the cached-ReadOptions change dominated the headline gain), but stacks cleanly and eliminates a 128 KB young-gen allocation per parse.
* **CORRECTNESS**: `CharStreamTokenizer` strict mode now rejects trailing-decimal numbers (`1.`, `1.e2`) per RFC 8259's `frac = "." 1*DIGIT` rule. Previously `strictJson` mode silently accepted `1.` because `Double.parseDouble("1.")` returns 1.0. A `seenDigitAfterDot` tracker in both `readNumberContinuation` and `readNumberGeneral` now enforces the rule when strict mode is on; non-strict (JSON5/lenient) mode still accepts `1.` → 1.0. 6 new `JsonTokenizerTest` cases cover both modes with regression-anchors for `1.5`, `42`, and `1e2` to confirm valid numbers still parse.
* **CORRECTNESS**: `CharStreamTokenizer.getBooleanValue()` and `getNumberType()` now reject calls on the wrong token type instead of silently returning stale typed-field state. `getBooleanValue()` previously returned `false` for any non-boolean token (the default of the `booleanValue` field) and could leak a stale value from a prior boolean token; `getNumberType()` similarly returned whatever `numberType` was last set. Both now throw `JsonIoException` with a "Boolean value requested" / "Numeric value requested" message. `JsonTokenizer` Javadocs updated with explicit `@throws` clauses to match the typed-numeric accessors. 5 new `JsonTokenizerTest` cases cover the rejection paths plus a happy-path regression-anchor.
* **PERFORMANCE**: `CharStreamTokenizer.readUnquotedIdentifier` now routes through the per-tokenizer string cache (same path quoted field names use), so repeated JSON5 unquoted identifiers — `{name: 1, name: 2, ...}` — intern to the same `String` instance instead of allocating fresh on every occurrence. New `unquotedFieldName_repeatedIdentifiersAreInterned` test asserts identity-equality across three repetitions to prove the cache is hitting.
* **DOCUMENTATION**: `JsonTokenizer.getText()` Javadoc clarified for numeric tokens: returns the canonical form (`Long.toString` / `Double.toString` / `BigInteger.toString` / `BigDecimal.toString`), not the raw source lexeme. `1e2` reads back as `"100.0"`, `1.2300` as `"1.23"`, `0xFF` as `"255"`, `+7` as `"7"`. Preserving the raw lexeme was prototyped and measured ~5% slower on `JsonPerformanceTest` (extra `numBuf.toString()` allocation per non-trivial number); shipping canonical form keeps the lazy-currentText win intact. Four regression-anchor tests in `JsonTokenizerTest` pin the documented behavior.
* **PERFORMANCE**: `JsonParser.readJsonObject` no longer probes the `substitutes` HashMap for full `@`-keys (`@type`, `@id`, `@items`, `@keys`, `@ref`, `@enum`). The map only contains `@x` short forms (length 2) and `$`-prefixed forms; full `@`-keys are guaranteed-misses. New gate: probe iff `firstCh == '$'` OR (`firstCh == '@'` AND length == 2). On default-options output (`cycleSupport=true`, `showTypeInfoMinimalPlus`) every `@type` / `@id` field now bypasses the HashMap.
* **CORRECTNESS**: `CharStreamTokenizer.nextToken()` now clears `currentToken` to `null` on the EOF transition. The {@link JsonTokenizer#currentToken()} Javadoc says "returns the token ... or null ... if the cursor is at EOF", but the previous implementation left `currentToken` pinned to the last-emitted token after `nextToken()` returned null. Two new `JsonTokenizerTest` cases (`currentToken_isNullAfterEof` and `currentToken_isNullBeforeFirstAdvance`) pin both halves of the contract.
* **CORRECTNESS**: `CharStreamTokenizer.getIntValue()` and `getLongValue()` now correctly enforce overflow contracts and match Jackson's `ParserBase` semantics. Previously the LONG → int branch silently truncated (despite Javadoc saying "throws if value does not fit"), and the BIG_INTEGER/BIG_DECIMAL branches threw on fractional values via `intValueExact()` (stricter than Jackson). Now: overflow / NaN / Infinity throw `JsonIoException`; fractional parts of in-range doubles are silently truncated like Java's `(int)` cast (`1.5 → 1`). 9 new tests in `JsonTokenizerTest` cover both methods across LONG, BigInteger, BigDecimal, double, NaN, and Infinity. Cursor API is package-private until 4.104.0+; behavior change has no impact on the existing tree-builder path (which calls `getDoubleValue()` / `getLongValue()`-on-LONG, never the lossy paths).
* **PERFORMANCE**: `CharStreamTokenizer` — number parsing now writes directly into typed instance fields instead of routing through a boxed `Number`. The chain `readNumber` → `emitNumber(Number)` → `JsonParser.materializeNumber()` previously did 2 boxes + 1 unbox per parsed number (autobox at `return n;`, cast/unbox in `emitNumber`, autobox again in `getLongValue()`/`getDoubleValue()`). Five new direct setters (`setLongResult`, `setDoubleResult`, `setFloatResult`, `setBigIntegerResult`, `setBigDecimalResult`) populate `longValue`/`doubleValue`/`numberType`/`currentToken` in one step; `emitNumber` is reduced to a fallback dispatcher used only by `parseToMinimalNumericType`. Median-of-3 JsonPerformanceTest vs Jackson: Maps Read 1.97x → 1.91x (**-3.3%**, -350 ms); POJO Read 1.38x → 1.34x (**-2.7%**, -156 ms). Writer paths also moved 2-5% faster (likely from reduced warmup-phase GC pressure in the same JVM); reported as secondary signal.
* **BUILD**: Bumped `java-util` runtime dependency 4.102.0 → 4.103.0.
* **REFACTOR**: `JsonParser`/`CharStreamTokenizer` — narrowed several `CharSequence` declarations to `String` where every actual value at the site was already a `String`. The `substitutes` map (was `Map<CharSequence, CharSequence>`, but lookups only worked for `String` keys due to the equals/hashCode contract) is now `Map<String, String>`; field-name handling in `readJsonObject` binds to the faster `StringUtilities.equals(String, String)` overload at six sites; two redundant `.toString()` calls (`name.toString()` after `readString`/`readUnquotedIdentifier`, and `value.toString()` in `emitString`) are gone. Also widened `readString`, `readStringSlowPath`, `readStringWithEscapes`, `cacheString`, and `cacheStringFromChars` return types from `CharSequence` to `String` (every return path was already a `String`). Pure type-system cleanup; behavior unchanged.
* **TESTING**: `SecurityAttackSimulationTest.simulateTimingAttack` rewritten to actually probe timing-side-channel disclosure: all 5 inputs now have identical byte length and JSON shape (differing only in field-name and value content), so a real timing leak through field-name lookup would still surface but size-driven variance no longer trips the test. Added a length-equality sanity check, a deeper warmup, best-of-3 trials per input, and tightened the variance threshold from 300% to 100%. The pre-existing flake (failing at 370–470% on busy machines) is gone.
...
* **PERFORMANCE**: `JsonIo.toJson(Object, WriteOptions)` — swapped the byte-based output pipeline (`FastWriter` → `OutputStreamWriter(UTF-8)` → `FastByteArrayOutputStream` → `new String(bytes, UTF-8)`) for a direct char-based `StringBuilderWriter` → `StringBuilder` → `sb.toString()` pipeline. Roughly half of JSON write time was in the char→byte→char round-trip. New `StringBuilderWriter` helper. `toJson(OutputStream, ...)` unchanged. JsonIo Write `cycleSupport=false` toJava **-10.3%**; best ratio vs Jackson 2.05x → 1.84x.
...
* **PERFORMANCE**: `JsonWriter.writeImpl()` — primitive-wrapper fast path for `Integer`/`Long`/`Boolean` when no type info or format pattern is active. Previously routed through `writeUsingCustomWriter` → `writeCustom` → `PrimitiveValueWriter.writePrimitiveForm`, which allocated `obj.toString()`. Now streams digits directly via `writeIntDirect`/`writeLongDirect` or cached `SMALL_INT_STRINGS`. `cycleSupport=false` toJava **-8.5%**; best ratio 1.84x → 1.68x.
...
* **PERFORMANCE**: `JsonValue.setType()` — fast path for `Class` types (which never carry unresolved type variables) skips the per-call `ConcurrentHashMap.computeIfAbsent()` validation cache for the 95%+ common case. Read times improved ~2.6% (toJava) and ~3.4% (toMaps).
* **PERFORMANCE**: `ReadOptions.isNonReferenceableClass` / `isNotCustomReaderClass` — now memoized via per-instance `ClassValue<Boolean>` caches, mirroring the existing `WriteOptions.nonRefCache` pattern. Previously re-ran `Set.contains` + three `isAssignableFrom` walks + `isEnum()` + `AnnotationResolver.getMetadata` on every call (~18% of Resolver samples). JsonIo Read toJava 1.99x → 1.97x; Toon Read toJava 2.05x → 2.00x.
* **PERFORMANCE**: `ObjectResolver.traverseFields` / `assignField` / `assignJsonObjectField` — hoisted the per-object `AnnotationResolver.getMetadata(target.getClass())` lookup out of the per-field loop. `parentMeta` is invariant across all fields of the same object but was re-derived per field (~20% of Resolver-phase samples). After this change, **all 8 Read/Write ratios vs Jackson are below 2.0x**.
* **PERFORMANCE**: `AnnotationResolver.getMetadata` — split into a tiny inlineable fast path and a `scanAndCache` cold-path helper. The combined method's try/finally bytecode inflated method size and suppressed HotSpot inlining at hot call sites in `ObjectResolver`/`WriteOptionsBuilder`/`WriteFieldPlan`. Fast-path self-time dropped ~50%.
* **PERFORMANCE**: `Injector` precomputes a `fastPath` byte tag at plan-build time (PRIMITIVE_NUMERIC/BOOLEAN/STRING/GENERIC). `ObjectResolver.assignField()` checks this first, short-circuiting primitive numeric/boolean/String assignments before the 5-case dispatch chain. Best toJava ratio 2.01x → 1.95x.
* **PERFORMANCE**: `ReadOptionsBuilder.buildInjectors()` — switched cached injector map from `LinkedHashMap` to `HashMap`. Nothing iterates it (all callers use `.get(fieldName)`), so insertion-order tracking was pure overhead. `MapResolver.traverseFields` toMaps reads improved ~2.5%.
* **PERFORMANCE**: `Resolver.DefaultReferenceTracker` — `HashMap` for `@id`/`@ref` tracking is now allocated lazily on first `put()`. Most JSON has no references, so the map is typically never created.
* **PERFORMANCE**: `isNonReferenceableClass` / `isNotCustomReaderClass` / `isNotCustomWrittenClass` — switched to the new `ClassValueSet.containsClass(Class)` fast path (java-util 4.101.0). Skips the `instanceof Class` guard and the post-`build()` unmodifiable-view delegation that `Set.contains(Object)` triggered. A typed fast-path reference is captured in `build()` before the view reassignment; pre-`build()` builder callers still route through `Set.contains`.
* **PERFORMANCE**: Hot-path `ClassValueMap.get(Object)` call sites switched to the new `ClassValueMap.getByClass(Class)` fast path (java-util 4.101.0), which skips the non-`Class` key guard and compiles to a near-direct `ClassValue.get(type)`. Applied across `AnnotationResolver`, `Resolver`, `Injector`, `JsonWriter`, `ReadOptionsBuilder`, `WriteOptionsBuilder`, and `Converter` outer-tier lookups. No semantic change.
* **REFACTOR**: `ToonReader.readInlineObject` parses the first field directly from `lineBuf` coordinates instead of materializing a `String firstFieldLine`. Callers pre-check the hyphen-line slice with buffer-based `findColonInBuf`, eliminating the per-element String allocation plus redundant scans. Consolidates two `parseCombinedArrayField` overloads into one. Benchmark impact was within noise (the test data favors tabular reads); shipped for code hygiene.
* **NOTE**: `JsonObject` internal storage refactored — eliminated `values[]`, `items[]`, and the `effectiveValues` pointer (three `Object[]` fields) in favor of `data[]` (POJO/Map entries) and `itemsRef` (externally-provided `@items` array). The fragile `effectiveValues` sync is replaced by a `valueArray()` method selecting on `storageMode`. Public API unchanged. Per-instance memory reduced by one `Object[]` reference. 9 new tests for the storage paths.
* **NOTE**: One-time `java.util.logging` warning when a Map with stringify-able keys (Long, UUID, etc.) is written using `@keys`/`@items` because `stringifyMapKeys` is off. Fires at most once per JVM lifetime and recommends `stringifyMapKeys(true)` or `standardJson()`. Prepares users for the default change in 5.0.
...
* **DEPENDENCY**: Requires java-util **4.101.0**, which ships a major internal `Converter` refactor. The four internal caches (`CONVERSION_DB`, `FULL_CONVERSION_CACHE`, `USER_DB`, `cacheInheritancePairs`) are now nested `ClassValueMap<ClassValueMap<V>>` keyed by (source class, target class), eliminating per-lookup `ConversionPair` allocation from json-io's read-side type-conversion hot path.
* **TESTING**: Added `StringKeyConversionTest` — 13 tests verifying deserialization of standard JSON with String map keys (e.g., `{"100": "value"}`) into typed `Map<K, V>` for K = Long, Integer, Double, Boolean, BigDecimal, BigInteger, UUID, Date, ZonedDateTime, AtomicLong, Character. Also covers backward compat with `@keys`/`@items` and empty maps. The underlying capability (from 4.94.0) was previously untested.
* **TESTING**: Expanded `JsonPerformanceTest` test data to cover floating-point fields, negative numbers, escapes, BigInteger/BigDecimal, java.time types, UUIDs, TreeMap, nullable fields, deep nesting, and a secondary POJO. Registered `JavaTimeModule` on Jackson so it can serialize java.time types. Establishes a more representative baseline.
* **TESTING**: Added JaCoCo coverage tests targeting gaps across the codebase — `JsonParserCoverageTest` (33), `JsonWriterCoverageTest` (36), `ToonWriterCoverageTest` (39), `ToonReaderCoverageTest` (33), `ResolverCoverageTest` (18), `ObjectResolverCoverageTest` (23), `MetaUtilsCoverageTest` (31), `AnnotationResolverCoverageTest` (18), `InjectorCoverageTest` (17).
