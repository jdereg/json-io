...
#### 4.102.0 - (Unreleased)
* **PERFORMANCE**: `JsonParser` trims additional `pushback()` calls from number continuation, negative-number fallback, unquoted JSON5 field names, and block-comment scanning while preserving token-boundary semantics.
