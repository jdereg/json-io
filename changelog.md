...
* **FEATURE**: Added `WriteOptionsBuilder.standardJson()` convenience method. Sets `showTypeInfoNever()`, `showRootTypeInfo(false)`, `cycleSupport(false)`, and `stringifyMapKeys(true)` in one call — producing standard JSON output identical to Jackson for normal usage (POJOs, Lists, Maps with String/numeric/UUID/Enum keys). These will become the defaults in json-io 5.0.0. Chainable — individual settings can be overridden after this call (e.g., `.standardJson().stringifyMapKeys(false)`).
