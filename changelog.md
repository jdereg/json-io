...
#### 4.101.0 - 2026-04-19
* **PERFORMANCE**: `JsonWriter.writeJsonUtf8String` (standard + JSON5 single-quoted overloads) and `ToonWriter.writeEscapedString` — bulk-copy the input `String`'s chars into a local `char[]` via the JDK's SIMD-intrinsic `String.getChars(0, len, buf, 0)`, then run the per-char escape-scan against the array instead of `s.charAt(i)`. Eliminates the per-character compact-string coder branch (LATIN1/UTF16), the per-char bounds check, and the method dispatch — straight-line array access in the hot scan loop. For string-value writes (the most common JSON write operation), typical input lengths (20-200+ chars for descriptions, URLs, content payloads, UUIDs, etc.) comfortably clear the ~7-char break-even, and the scan loop is tight enough that the relative savings per char are meaningful. A **local** `char[]` allocation (not the `StringUtilities.getChars` ThreadLocal buffer) is used deliberately: these methods invoke `output.write(...)` inside the scan loop on each escape hit, and the output `Writer` is a user-supplied extension point — a custom `Writer` implementation calling any method in the `StringUtilities.getChars` consumer chain would clobber a shared ThreadLocal buffer and produce garbled output. A local buffer is reentrancy-safe by construction regardless of Writer implementation. The SIMD bulk-copy benefit is identical either way; the only cost is a single TLAB-served char[] allocation per call (tiny, young-gen collected immediately). For the JSON5 overload, `s.codePointAt(i)` is replaced with the equivalent `Character.codePointAt(buf, i)` — same surrogate-pair semantics. All 3806 tests pass.
...
* **TESTING**: Expanded `JsonPerformanceTest` test data to cover floating-point fields, negative numbers, escapes, BigInteger/BigDecimal, java.time types, UUIDs, TreeMap, nullable fields, deep nesting, and a secondary POJO. Establishes a more representative baseline that prevents narrow optimizations from being mistaken for real-world wins. Registered `JavaTimeModule` on Jackson so it can serialize java.time types.
* **TESTING**: Added `JsonParserCoverageTest` — 33 tests targeting JaCoCo gaps in `JsonParser` (parsing edge cases for strings, numbers, objects, arrays, whitespace, and error paths).
* **TESTING**: Added `JsonWriterCoverageTest` — 36 tests targeting JaCoCo gaps in `JsonWriter` dispatch paths.
* **TESTING**: Added `ToonWriterCoverageTest` — 39 tests targeting JaCoCo gaps in `ToonWriter` (field-entry variants, inline/block output, primitive/collection/map dispatch).
* **TESTING**: Added `ToonReaderCoverageTest` — 33 tests targeting JaCoCo gaps in `ToonReader` (header parsing, number parsing edge cases, string escape handling, error paths).
* **TESTING**: Added `ResolverCoverageTest` — 18 tests exercising type-coercion paths in `Resolver`.
* **TESTING**: Added `ObjectResolverCoverageTest` — 23 tests targeting JaCoCo gaps in `ObjectResolver` field/element assignment paths.
* **TESTING**: Added `MetaUtilsCoverageTest` — 31 tests targeting JaCoCo gaps in `MetaUtils` (excluding deprecated methods).
* **TESTING**: Added `AnnotationResolverCoverageTest` — 18 tests for `@Io*` annotation resolution paths (in `com.cedarsoftware.io.reflect`).
* **TESTING**: Added `InjectorCoverageTest` — 17 tests targeting JaCoCo gaps in field-injection dispatch (in `com.cedarsoftware.io.reflect`).
