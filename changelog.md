...
* **MAINTENANCE**: `ObjectResolver.assignField()` now gates scalar converter execution with `converter.isConversionSupportedFor(source, target)` (while retaining simple-target filtering) instead of `isSimpleTypeConversionSupported(source, target)`, reducing dependence on the pair-form "simple type" API without changing behavior.
