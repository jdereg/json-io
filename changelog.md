...
* **PERFORMANCE**: **Factory-LMF VarHandle binding repairs `Accessor`'s primitive-field fast path.** Latent bug: `LambdaMetafactory` rejects `getField` MethodHandle kinds, so the primitive-lambda path added to Accessor has been silently dead since introduction — every primitive-typed field accessor autoboxed on every read. Fix: build 8 factory `MethodHandle`s via LMF at static init, one per primitive type, binding a `VarHandle` into an `XGetter` functional-interface instance. Per-call cost: ~3.36 ns/getInt (JIT fully inlines). JDK 8 source-compatibility preserved (no `VarHandle` import; runtime access through `Class<?>` loaded via `Class.forName`, gated by JDK ≥ 9). JFR allocation impact: `Integer` wrappers from `Accessor.retrieve` eliminated, `Double` eliminated, `Long` reduced ~80%.
