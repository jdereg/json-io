...
#### 4.102.0 - (Unreleased)
* **PERFORMANCE**: `JsonParser` now carries parse depth as a local recursive argument instead of incrementing/decrementing a parser member for every object/array nesting. Also trimmed additional `pushback()` calls from number continuation, unquoted JSON5 field names, and block-comment scanning while preserving token-boundary semantics.
