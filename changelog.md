...
#### 4.103.0 - (Unreleased)
* **FEATURE**: Added `CharStreamGenerator.writeFieldNameRaw(String preformattedKeyAndColon)` — package-private fast-path hook for emitting key+colon pairs that the caller has already formatted (the precomputed-prefix pattern used by JsonWriter for `@items` / `@keys` meta-key emission). Skips the per-call quote-decision + escape-scan that `writeFieldName(String)` performs. Transitions state from `FRAME_OBJECT_EMPTY` / `FRAME_OBJECT_AFTER_VALUE` to `FRAME_OBJECT_AFTER_FIELD`. Does NOT emit a separator — the caller is responsible for any leading comma. Same pattern as `writeStringFieldUnescaped` (state-machine-aware fast path for known-safe input), now applied to the field-name side. Foundation for the upcoming `writeImpl` structural-emission migration (steps 4+ of the dog-food migration). 1 new test in `JsonGeneratorTest` covers the round-trip. All 4084 tests pass; 0 failures.
