...
#### 4.101.0 - unreleased
* **PERFORMANCE**: java-util `Converter.getCachedConverter()` — added an instance-level 1-slot cache for the most-recent (source, target, result) tuple. JFR profiling showed `ConversionPair` allocation as a measurable hotspot during `toMaps`/`toJava` reads of diverse data types (UUID, Date, Instant, BigDecimal). Repeated lookups with the same type pair (common when iterating homogeneous collections like `List<UUID>` or `List<Date>`) now skip both the `ConversionPair` allocation and the `ConcurrentHashMap.get()` entirely. Read time improved ~3.7% on `toMaps` for a workload with diverse data types. Requires java-util ≥ 4.101.0.
