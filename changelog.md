...
#### 4.104.0 - (Unreleased)
* **PERFORMANCE**: Temporal fast paths extended to TOON write and maps-mode read. `ToonWriter`'s CONVERTER_SUPPORTED emission uses the hand-rolled `TemporalChars` emitters (new `ISO_ZONED_DATE_TIME`-semantics variant: zone id bracketed only for region zones, matching java-util's Converter output exactly — parity pinned against the live Converter) instead of `Converter` → `DateTimeFormatter`, which JFR showed at ~11% of toon-write CPU; strings still flow through `writeString` so TOON quoting is unchanged. `MapResolver` (field coercion + array elements) gains the same strict-ISO `FastIsoParser` hook the typed path got, with identical Converter fallback. TOON Write **-11.8%/-13.6%** (cycle on/off), TOON Read **-6.1%**, median of 3 with full range separation; JSON paths flat as controls. (Also investigated: `getNumberValue` boxing — required by the JsonObject tree until eager construction; widening boxing caches previously measured negative.)
* **PERFORMANCE**: Class-classification caching on the write hot path. `WriteFieldPlan` memoizes the `isForceType` answer per runtime class (fields are overwhelmingly monomorphic; the computation consults several class-keyed sets per field — JFR showed the family at ~5% of write CPU). `ToonWriter.writeInlineValue` gains the same instanceof fast paths as `isPrimitive`, eliminating a double ClassValue dispatch per inline element. JSON Write **-5.1%/-7.1%** (cycle on/off), TOON Write **-6.0%/-7.1%**, median of 3 with full range separation.
* **PERFORMANCE**: String-emission fast paths in the write pipeline. (1) `writeJsonUtf8String` scans the String directly and emits safe runs as String slices — the sink performs one `String.getChars` into its buffer, removing the prior `CharBufScratch` ThreadLocal staging copy per string value (JFR: ~4.8% of write CPU). (2) New hand-rolled ISO-8601 emitters (`TemporalChars`) replace `DateTimeFormatter` in the LocalDate/LocalTime/LocalDateTime/ZonedDateTime/OffsetDateTime/OffsetTime/Instant writers — digits written straight into a `char[]` and emitted via a new unescaped char[] value path; formatter fallback retained for years outside 0..9999. Byte-for-byte output parity pinned by 60k+ randomized comparisons. Combined: JSON Write **-5.5%/-7.4%** (cycle on/off), median of 3 with range separation.
* **PERFORMANCE**: Per-instance cache-compare scratch in `CharStreamTokenizer` and `ToonReader`, replacing a `CharBufScratch` ThreadLocal borrow on every cache-checked string (JFR: `ThreadLocalMap.getEntry` at ~8% of JSON read-phase CPU, more on TOON). Both readers are per-parse single-threaded, so a plain 64-char field (bounded by `MAX_CACHED_STRING_LENGTH`) replaces the ThreadLocal map walk with a field load. JSON Read **-2.4%** (1.67x → 1.62x vs Jackson), TOON Read **-4.8%** (1.78x → 1.73x), median of 3.
* **PERFORMANCE**: **Strict-ISO-first temporal parsing on the read side (`FastIsoParser`).** String → java.time conversion previously routed through `Converter` into `DateUtilities.parseDate` — a flexible, regex-driven parser that JFR showed at ~13% of read-phase CPU, re-parsing the strict ISO-8601 strings json-io itself writes. The read path (`ObjectResolver.assignField` + `readWithFactoryIfExists`) now tries the strict `java.time` parser first for Instant/LocalDate/LocalDateTime/LocalTime/ZonedDateTime/OffsetDateTime/OffsetTime/Duration/Period/Year/YearMonth/MonthDay/ZoneOffset/ZoneId, falling back to the Converter path on any parse failure — all flexible formats ("January 5, 2024", etc.) work exactly as before. JSON Read **-9.1%** (1.82x → 1.67x vs Jackson), TOON Read **-3.5%** (1.88x → 1.78x), median of 3.
* **PERFORMANCE**: Escape-free string fast path for built-in writers whose output alphabet can never need JSON escaping. New package-private `JsonGenerator.writeStringUnescaped(String)` (quote + bulk write + quote — no escape scan, no scratch-buffer staging); adopted by the default paths of the java.time writers (LocalDate/LocalTime/LocalDateTime/ZonedDateTime/OffsetDateTime/OffsetTime/Instant/Duration/Period/Year/YearMonth/MonthDay/ZoneOffset), Date/sql.Date/Timestamp/Calendar, UUID, BigInteger, BigDecimal, Currency, and Locale. User-supplied format patterns retain the escaping path.
* **PERFORMANCE**: Class-keyed hot-path lookups moved from hash-probe sets to java-util's `ClassValueSet`/`ClassValueMap` (JVM ClassValue fast path): `Primitives.PRIMITIVE_WRAPPERS`/`NATIVE_JSON_TYPES` and JsonWriter's `CONVERTABLE_TYPES`/`NUMERIC_PRIMITIVES_FOR_COMPACT`/`NATURAL_DEFAULTS` — all consulted by `isForceType` per field. Combined with the escape-free fast path above: JSON Write **-1.6%** (1.71x → 1.67x vs Jackson, cycleSupport=true) / **-2.1%** (1.56x → 1.53x, cycleSupport=false), median of 3.
