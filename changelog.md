...
* **PERFORMANCE**: `WriteFieldPlan.toonKeyLiteral` — pre-computed TOON key literal (`"fieldName: "` or `"\"fieldName\": "`) at plan-build time, reused across all ToonWriter calls. `ToonWriter.writeObjectFields()` uses this in its primitive/String fast path to write the key + colon + space as a single `out.write(toonKeyLiteral)` call, eliminating the per-field `writeKeyString()` → `needsQuoting()` → `quoteDecisionCache.get()` dispatch chain. Mirrors the existing JSON `serializedKey` pattern. Java POJO field names are identifiers that virtually never need TOON quoting, so the pre-computed literal is `fieldName + ": "` for 99.9% of fields. Performance-neutral on the current benchmark (the `needsQuoting` cache was already fast), but reduces per-call overhead and method dispatch for workloads with many POJO fields.
