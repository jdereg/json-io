...
* **PERFORMANCE**: `ObjectResolver.traverseFields` — adaptive shape cache maps a JsonObject's JSON-key sequence to a position-indexed `FieldAssignmentPlan[]` after `SHAPE_CACHE_THRESHOLD` (=2) consecutive same-shape calls, skipping the per-field `assignmentPlansByName.get(key)` HashMap lookup on subsequent objects of the same shape. Identity-based key comparison is safe because the parser interns field-name Strings. Threshold tuned empirically via a targeted micro-benchmark plus a java-json-benchmark cross-threshold sweep. JsonPerformanceTest median-of-3: Read **-1.8%** (1.80x → 1.78x Jackson). java-json-benchmark publication-config sweep, USERS DATABIND deser ratio vs Jackson: 10KB **5.28x → 4.51x** (+14.5% throughput), 100KB **5.68x → 4.72x** (+12.7%), 1MB **5.83x → 4.85x** (+19.3%). CLIENTS DATABIND deser flat (Converter cost per field hides cache delta). Other cells unaffected (write-side and Maps mode bypass this code path).
