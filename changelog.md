...
#### 4.102.0 - (Unreleased)
* **PERFORMANCE**: TOON uniform-POJO tabular detection now uses `WriteFieldPlan` primitive metadata to treat primitive fields as always present and inline-safe, avoiding `Accessor.retrieve()` boxing during the prepass for primitive-only fields. Tabular row writing still uses the primitive Accessor getters added below. Added counter-based tests proving primitive getters are invoked only during row writing, not during collection/array uniformity detection. Follow-up JFR (`JsonPerformanceTest_2026_04_26_115406.jfr` vs `...113105.jfr`) showed the prepass change as a CPU/path-shape win: total execution samples -2.6%, JSON/TOON write samples -3.0%, TOON write samples -3.2%, and weighted allocation effectively flat.
* **PERFORMANCE**: `Accessor` now exposes primitive getter APIs for all eight Java primitive types and builds primitive `LambdaMetafactory` getters for primitive fields/getters when available. Existing `retrieve()` behavior remains boxed-compatible, while JSON and TOON field writers now use the primitive getters to write primitive POJO fields without boxing on the direct-write path. TOON tabular POJO rows use the same primitive path. Added coverage for all eight primitive field/method accessors and JSON/TOON primitive field output. Follow-up JFRs (`JsonPerformanceTest_2026_04_26_112159.jfr` and `...113105.jfr`) were safe overall: the two-run average was ~0.9% fewer execution samples than the clean post-guard baseline (`...171755.jfr`), with JSON/TOON write samples down ~1.2%, read samples down ~1.7%, and weighted allocation effectively flat.
* **BUG FIX**: `JsonParser.readString()` now materializes borrowed `FastReader.BufferSlice` content before any subsequent reader operation can refill and overwrite the internal buffer, and TOON line parsing now releases borrowed line slices at safe line/parse boundaries. This closes a rare root-string/escape edge case where a short borrowed string segment near the end of the reader buffer could be corrupted by trailing whitespace or escape handling before it was cached/appended. Added regression coverage that forces buffer refills across the borrowed-slice boundary. Follow-up `JsonPerformanceTest` JFR comparison (`JsonPerformanceTest_2026_04_25_171755.jfr` vs `...161536.jfr`) showed no measurable guard overhead: overall samples/sec stayed effectively flat (49.83 → 49.83), normalized read-phase shares were within noise (toJava JSON +0.034 pp, toJava TOON +0.050 pp, toMaps JSON -0.109 pp, toMaps TOON -0.099 pp), and no `BufferSlice.release()` / assertion-helper leaves appeared.
