...
* **CORRECTNESS**: **TOON v3.3 spec conformance — the full upstream fixture suite now passes.** `ToonWriter` and `ToonReader` corrected against the official spec at [toonformat.dev](https://toonformat.dev/) v3.3 across §2 (numbers), §4 (type inference), §7.1 (escape sequences), §7.3 (key encoding), §8 (objects), §9.1 / §9.3 / §9.4 (arrays — empty, tabular, deeply-nested list), §12 (whitespace / blank lines / custom indent), §13 (defaults), §13.4 (key folding / opt-in path expansion), and §14 (strict-mode validation: duplicate keys, trailing content, malformed brackets). Notable behavior changes: uniform-`Map` arrays now emit in tabular form (CSV-like header + rows); `Long`-overflow numeric literals decode as `BigInteger` instead of silently truncating via `bigInt.longValue()`; quoted strings decode `\uXXXX` escapes; leading-zero forms (`007`) decode as strings; bare-hyphen lines decode as empty objects; tabular header keys are quote-aware. New `ToonSpecFixtureTest` drives the vendored upstream fixtures (under `src/test/resources/toon-spec-fixtures/`, refreshable at a pinned commit via `scripts/refresh-toon-fixtures.sh`) with a bidirectional expected-failures mechanism — listed-fails-skip and listed-passes-fail-loud, so both silent regression and silent over-fixing surface in CI.
