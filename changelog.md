...
#### 4.102.0 - (Unreleased)
* **PERFORMANCE**: `JsonWriter.writeJsonUtf8String()` — bulk-copies the input string into a per-thread `char[]` via the SIMD-intrinsic `StringUtilities.getChars()`, then walks `buf[i]` (raw array load) instead of `s.charAt(i)` (per-character `StringLatin1`/`UTF16` dispatch). Slice writes route through `output.write(buf, off, len)` → `sb.append(char[], 0, len)` (the fastest `StringBuilder` variant) instead of `sb.append(String, off, off+len)`. JFR profiling showed `String.charAt` + `StringLatin1.charAt` at ~345 leaf samples combined inside this loop. JsonPerformanceTest 3-run medians: JsonIo Write `cycleSupport=false` toJava 4228 → 4138 ms (**-2.14%**), toMaps 4252 → 4149 ms (**-2.40%**); JsonIo Write `cycleSupport=true` (also routes through `writeJsonUtf8String`) toJava 4740 → 4558 ms (**-3.84%**), toMaps 5178 → 5018 ms (**-3.08%**). Re-entrancy contract documented inline (TL `char[]` consumed synchronously by `output.write` calls before method returns; standard writers do not transitively invoke `StringUtilities.getChars`).
* **PERFORMANCE**: `ToonReader.STRING_CACHE_MASK` — grew the per-thread string cache from 2048 to 4096 slots (`STRING_CACHE_MASK` 2047 → 4095). The `TL_STRING_CACHE` `withInitial` lambda auto-resizes via `STRING_CACHE_MASK + 1`. Trades ~16 KB extra per thread (one-time) for fewer eviction collisions in workloads with high-cardinality field-value strings. JsonPerformanceTest 3-run medians: TOON Read toMaps 7057 → 6904 ms (**-2.16%**); Full Java essentially flat (typed primitives bypass the cache, so most Full Java reads don't pressure it).
* **PERFORMANCE**: `JsonIo.toJson()` / `JsonIo.toToon()` — initial `StringBuilder` capacity raised from `DEFAULT_CHAR_BUFFER_SIZE` (8192) to 32768 chars. Skips several early `ensureCapacityInternal` doublings on typical payloads. JFR showed `ensureCapacityInternal` at ~276 leaf samples on the write paths; samples dropped ~16% post-change. Both `toJson(Object, WriteOptions)` and `toToon(Object, WriteOptions)` updated; the existing `DEFAULT_CHAR_BUFFER_SIZE` constant (also used for `char[]` read buffers) is left untouched. JsonPerformanceTest 3-run medians: TOON Write `cycleSupport=false` toJava 4719 → 4525 ms (**-2.09%**), toMaps 4798 → 4487 ms (**-2.76%**); collateral wins on every other write metric (TOON Write `cycleSupport=true` toJava **-3.38%**, toMaps **-3.12%**).
* **PERFORMANCE**: `ToonWriter` — replaced 21 instances of `out.write(": ")` with two single-character writes (`out.write(':'); out.write(' ');`) and 46 instances of `out.write(NEW_LINE)` (where `NEW_LINE = "\n"` is hardcoded per TOON spec) with `out.write('\n')`. Each `String`-overload of `StringBuilder.append` triggers `String.getBytes` + `ensureCapacityInternal`; single-character append takes a tighter `putCharsAt` path. JFR showed those leaves at #1 and #2 overall (354 + 276 samples) before the change, 247 + 232 after. JsonPerformanceTest 3-run medians: TOON Write `cycleSupport=false` toJava 4719 → 4524 ms (**-4.13%**), toMaps 4798 → 4615 ms (**-3.82%**). Cross-platform: `NEW_LINE` was already hardcoded LF per TOON spec, not platform-dependent, so single-char `'\n'` is equivalent on Windows.
...
* **REFACTOR**: `JsonObjectMap` no longer shadows the parent's `mapKeyType` field. The subclass previously declared its own `private Type mapKeyType` plus `getMapKeyType()` / `setMapKeyType()` overrides, but `JsonObject` (the parent) already holds a `mapKeyType` field — and that parent field is the one written by `ObjectResolver.seedIncrementalContainerMetadata` when the JsonObject is a lite (String-keyed) map that hasn't been promoted to `JsonObjectMap`, and the one read by `JsonObject.copyMetadataFrom` during promotion. The shadow was a latent bug: any code path that obtained a `Type` via the lite-stored field on the parent and later tried to read it back through a `JsonObjectMap` reference (post-promotion) would have seen `null`, since the override returned the empty subclass field. Removed the duplicate field, both overrides, and the redundant `mapKeyType = null;` line in `JsonObjectMap.clear()` (the parent's `clear()` already handles the field). All 3,834 tests pass.
* **PERFORMANCE / REFACTOR**: `JsonParser.readArray` peeks for `]` up front, so the empty-array case never reaches `readValue` and never has to pushback `]` + return an `EMPTY_ARRAY` sentinel. Removed the static `EMPTY_ARRAY` field, the `case ']'` branch in `readValue`, and the per-iteration `value != EMPTY_ARRAY` check in `readArray`. One more pushback eliminated; cleaner control flow.
* **PERFORMANCE**: `JsonParser.readArray` and `readJsonObject` now hand the already-read first character of the next value/field straight to `readValue(int, Type)` and a new `readFieldName(int)` overload, instead of pushing it back into `FastReader` and re-reading it. Eliminates 3 read-then-pushback round-trips that fired on every non-trailing array element, every object field, and every empty-object peek — ~17M `getfield`/`putfield` operations on `pushbackPosition` and `pushbackBuffer` per JsonPerformanceTest run. JsonPerformanceTest 3-run medians: toJava Read 9689 → 9469 ms (**-2.3%**, ratio 1.86x → 1.85x Jackson), toMaps Read 5071 → 4823 ms (**-4.9%**, ratio 1.25x → 1.20x Jackson). The maxDepth check moved from no-arg `readValue(Type)` into `readValue(int, Type)` so all entry points enforce it uniformly.
* **PERFORMANCE**: Float/double/BigDecimal/BigInteger parsing now backed by the Eisel-Lemire algorithm via java-util 4.102.0's vendored Randelshofer FastDoubleParser. `JsonParser.readFloatingPoint` / `readInteger` BigInteger paths, `ToonReader.parseNumber` BigInteger fallbacks, and `ObjectResolver` scalar coercion all route through the new `MathUtilities.parseDouble/parseFloat/parseBigDecimal/parseBigInteger` entry points. Output is bit-identical to the JDK reference parsers (parity test in java-util). JsonPerformanceTest 3-run medians vs the pre-FastDoubleParser HEAD baseline: toMaps Read ratio 1.317x → 1.276x Jackson (**-3.1%**), toJava Read essentially flat (test data has few floats, so the win shows mostly on Maps mode where every number flows through `parseToMinimalNumericType`). JFR confirms ~50% reduction in JDK `FloatingDecimal` samples on json-io's path; ~35 new samples in `JavaDoubleParser`. Real-world payloads with more doubles will see proportionally larger wins.
* **REFACTOR**: `JsonObject` split into a shape-cut hierarchy: lite `JsonObject` (POJO / String-keyed map — public API) plus package-private `JsonObjectArray` (`@items`-shaped) and `JsonObjectMap` (complex-keyed `@keys`/`@items`). Lite `JsonObject` shed the `Object[] itemsRef` field and the `byte storageMode` (with its `MODE_*` constants and `valueArray()` indirection) — pure heap savings on every lite instance, the common case in deserialized graphs. `JsonParser` and `ToonReader` now peek through metadata fields and allocate the right subclass up front; `JsonObject.promoteToArray()` / `promoteToMap()` lazy-promote when a shape determiner (`@items`/`@keys`) arrives after non-meta fields, copying parent metadata and updating the `@id`/`@ref` table. Public API unchanged — external callers still hold `JsonObject` references. New `JsonObjectFamilyTest` (20 tests) pins cross-shape equality, hash consistency, and promotion contracts. JsonPerformanceTest median deltas vs the pre-refactor baseline are within run-to-run noise (≤ ±3.5% across all 16 tracked metrics, with Jackson alone moving ±2.7% across runs); JFR top-hot-method profile unchanged (no new hotspot from polymorphic dispatch); JsonObject family allocation count net-flat (lite count down, JsonObjectArray new, parser-scoped `PendingMeta` short-lived).
