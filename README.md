...
**json-io** consists of two main classes, a reader (`JsonReader`) and a writer (`JsonWriter`).  **json-io** eliminates the need for using `ObjectInputStream / ObjectOutputStream` to serialize Java and instead uses the JSON format.  There is a 3rd optional class (`JsonObject`) which implements `Map` and is used to represent objects when using the special 'Maps' API [JsonReader.jsonToMaps()] which reads from JSON and returns an object graph built of 'Map of Maps' as opposed to creating teh appropriate Java objects, which is the default behavior.
