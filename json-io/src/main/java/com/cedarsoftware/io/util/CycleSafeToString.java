...
/**
 * The one {@code toString()} walk every json-io container uses, so that none of them can die with
 * {@link StackOverflowError} on a cyclic graph -- and a graph read from JSON with {@code @ref} is routinely cyclic.
 * <p>
 * This is the same walk as java-util's (4.112.0+), carried here so json-io does not depend on that release.
 * <p>
 ...
 * outermost render. Every json-io container renders through this class, so the path is shared across all of
 ...
 * rendered by its own {@code toString()}, arrays included (never expanded). A wrapper renders its contents in
 * that same standard format.
 * <p>
 ...
 * synchronized or unmodifiable JDK wrappers, {@code Hashtable}, {@code Vector} -- never re-enters this class, so
 * it behaves exactly as calling {@code toString()} on those containers directly would.
 *
...
final class CycleSafeToString {
    ...
    /**
     * Renders a map entry in the JDK's format: {@code key=value}, with the key and the value rendered on this same
     * path, so a value that leads back into a container already being rendered closes as {@code (cycle)}.
     */
    static String entry(Map.Entry<?, ?> self) {
        return render(self, path -> {
            StringBuilder sb = new StringBuilder();
            append(sb, self.getKey(), path);
            sb.append('=');
            append(sb, self.getValue(), path);
            return sb.toString();
        });
    }
     ...
     * by its own {@code toString()}, and a json-io container re-enters {@link #render} on this same path.
     */
